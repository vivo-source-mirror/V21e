...
struct cluster_data {
	...
	unsigned int big_core_ctl; /*add for control big core switch,by vivo_pem 2018/1/11 */
	bool enable;
	int nrrun;
...
/* add for control big core switch by vivo_pem 2017/1/11 begin */
static ssize_t store_big_core_ctl(struct cluster_data *state,
				const char *buf, size_t count)
{
	...
	state->big_core_ctl = val;
	wake_up_core_ctl_thread(state);

	return count;
}

static ssize_t show_big_core_ctl(const struct cluster_data *state, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", state->big_core_ctl);
}
/* add for control big core switch by vivo_pem 2017/1/11 end */
...
core_ctl_attr_rw(big_core_ctl); /*add by vivo_pem 2017/1/11 */
...
static struct attribute *default_attrs[] = {
	&big_core_ctl.attr, /*add by  vivo_pem 2018/1/11 */
	&min_cpus.attr,
	&max_cpus.attr,
				    ...
				    unsigned int new_need)
{
	...
	/*add by vivo_pem 2018/1/11*/
	if (cluster->nrrun > (new_need + cluster->big_core_ctl))
		new_need = new_need + 1;
...
static int cluster_init(const struct cpumask *mask)
{
	...
	cluster->big_core_ctl = 0; /*add by vivo_pem 2018/1/11*/
