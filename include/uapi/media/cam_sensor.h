...
struct cam_cmd_probe {
	...
	/*luzhi add for thermal meter begin*/
	uint16_t    thermal_addr;
	uint16_t    thermal_mask;
	uint16_t    thermal_shift;
	/*luzhi add for thermal meter end*/
    char sensor_name[MAX_SENSOR_NAME_SIZE];     //vivo wenjun add for sensor probe debug 2020/6/11
} __attribute__((packed));
