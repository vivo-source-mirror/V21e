...
static int cms_check_charge_timeout(struct cms *chip, int chg_status_now, int dchg_status_now)
{
	...
	if (count_flag) {
		...
		if (timeout) {
			if (chip->no_chg_timeout || bsp_test_mode) {
		...
		if (adapter_power_derate_already.intval && !chip->adapter_power_derate_param_ready) {
			...
			if (adapter_power_derate.intval != 0) {
				...
				if ((adapter_power.intval & 0x0F) == 0x03) {//33W adapter
					...
					if ((adapter_power.intval & 0x0F) == 0x02) {//22.5W adapter
						active_current = 3400;
				...
				cms_print(chip, PR_INFO, "notice: trigger %d, release %d, active %d \n", trigger_temp, release_temp, active_current);
			} else {
...
static int cms_check_abnormal_charge_current(struct cms *chip, int batt_temp)
{
    ...
    if ((chip->enabled  && chip->battery_rc.bat_id == B_UNKOWN) || health) {
		batt_scale = 0;
...
static void cms_main_work(struct work_struct *work)
{
	...
	cms_print(chip, PR_INFO, "vbat=%d,continue_count=%d,nr_work=%d,fb_on=%d,temp=%d,chg_status=%d,usb_type=%d,chg_volt_v=%d,input_scale(%s)=%d,batt_scale(%s)=%d,batid=%d,health=0x%X,batt_soc=%d,bsptest=%d,fast_chg_state=%d,dchg_support=%d\n",
		...
		get_effective_result(chip->user_input_votable), get_effective_client(chip->user_ibat_votable), get_effective_result(chip->user_ibat_votable), chip->battery_rc.bat_id, (int)chip->health_status, batt_soc.intval, bsp_test_mode, fast_chg_state, dchg_support.intval);
out:
	if(!chip->health_status)
		work_period_ms *= 2;
...
static int cms_parse_dt(struct cms *chip)
{
	...
	if(!chip->normal_tc_rc || !chip->normal_tc_data){
		...
		chip->normal_tc_rc = default_normal_tc_data_rc;
	...
	if(chip->intell_charge_enable) {
		...
		if(!chip->primary_fbon_tc_rc || !chip->primary_fbon_tc_data){
			...
			chip->primary_fbon_tc_rc = default_primary_fbon_tc_data_rc;
		...
		if(!chip->parallel_fbon_tc_rc || !chip->parallel_fbon_tc_data){
			...
			chip->parallel_fbon_tc_rc = default_parallel_fbon_tc_data_rc;
		...
		if(!chip->primary_fboff_tc_rc || !chip->primary_fboff_tc_data){
			...
			chip->primary_fboff_tc_rc = default_primary_fboff_tc_data_rc;
		...
		if(!chip->parallel_fboff_tc_rc || !chip->parallel_fboff_tc_data){
			...
			chip->parallel_fboff_tc_rc = default_parallel_fboff_tc_data_rc;
	...
	if (chip->ex_intell_charge_enable) {
		...
		if (!chip->ex_primary_fbon_tc_rc || !chip->ex_primary_fbon_tc_data) {
			...
			chip->ex_primary_fbon_tc_rc = default_primary_fbon_tc_data_rc;
		}
		...
		if (!chip->ex_parallel_fbon_tc_rc || !chip->ex_parallel_fbon_tc_data) {
			...
			chip->ex_parallel_fbon_tc_rc = default_parallel_fbon_tc_data_rc;
		}
		...
		if (!chip->ex_primary_fboff_tc_rc || !chip->ex_primary_fboff_tc_data) {
			...
			chip->ex_primary_fboff_tc_rc = default_primary_fboff_tc_data_rc;
		}
		...
		if (!chip->ex_parallel_fboff_tc_rc || !chip->ex_parallel_fboff_tc_data) {
			...
			chip->ex_parallel_fboff_tc_rc = default_parallel_fboff_tc_data_rc;
		}
	}
	...
	if (chip->usb_connecter_protect_enable) {
		...
		if (!chip->usb_conn_heat_protect_data_rc || !chip->usb_conn_heat_protect_data) {
			...
			chip->usb_conn_heat_protect_data_rc = default_usb_conn_temp_heat_data_rc;
		}
	...
	chip->dp_batt_temp_enable = of_property_read_bool(node, "vivo,dp-batt-temp-enable");
	if (chip->dp_batt_temp_enable) {
		rc = of_property_read_u32(node, "vivo,dp-batt-temp-data", &chip->dp_batt_temp_data);
		if (rc) {
			chip->dp_batt_temp_data = 100;
		}
	}
