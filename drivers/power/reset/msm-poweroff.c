...
#ifdef CONFIG_VIVO_MINIDUMP_SUPPORT
...
#endif	/*CONFIG_VIVO_MINIDUMP_SUPPORT*/
...
static int dload_set(const char *val, const struct kernel_param *kp)
{
	...
	printk("[%s][%d] download_mode %d\n", __FUNCTION__, __LINE__, download_mode);
...
#if 0 /*  Don't disable sdi, Otherwise we can't get the key info of crash!!!*/
	if (!download_mode)
		scm_disable_sdi();
#endif
...
static int force_warm_reset(const char *val, const struct kernel_param *kp)
{
	pr_notice("Force a warm reset\n");
	/* Set reset type to warm reset */
	qpnp_pon_system_pwr_off(PON_POWER_OFF_WARM_RESET);

	deassert_ps_hold();
	msleep(10000);
	pr_notice("Force a warm reset failed\n");
	return 0;
}
module_param_call(warm_reset, force_warm_reset, param_get_int,
			&warm_reset, 0644);
...
#ifndef CONFIG_VIVO_MINIDUMP_SUPPORT
...
skip_sysfs_create:
	...
	/* Begin leiweiqiang modify download_mode dependent on bsp_test_mode. 2016-10-12*/
	if (is_debug_mode)
		download_mode = 1;
	printk(KERN_ERR "%s %d bsp_test_mode:%d download_mode:%d\n", __func__, __LINE__, bsp_test_mode, download_mode);
	/* End leiweiqiang modify download_mode dependent on bsp_test_mode. 2016-10-12*/
