...
static bool set_ssr_related;
//#define ENABLE_ADSP_CRASH
#ifdef ENABLE_ADSP_CRASH
...
static int adsp_restarted_count;
extern unsigned int download_mode;
...
void set_related_restart_level(void)
{
	set_ssr_related = true;
	pr_err("ADSP set_related_restart_level\n");
}
EXPORT_SYMBOL(set_related_restart_level);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < ARRAY_SIZE(restart_levels); i++)
		if (!strncasecmp(buf, restart_levels[i], count)) {
			...
			if (force_ssr_related & (1 << subsys->id)) {
				pr_info("%s: subsys%d force restart level to RELATED\n", __func__, subsys->id);
...
int subsystem_restart_dev(struct subsys_device *dev)
{
	...
	if (!strcmp(name, "adsp")) {
		adsp_restarted_count++;
		pr_info("subsys-restart: restart request for %s, count %d.\n",
				name, adsp_restarted_count);
	}
	...
	if (strnstr(name, "adsp", strlen(name))) {
#ifdef ENABLE_ADSP_CRASH
		temp_restart_level = dev->restart_level;
		...
		if (set_ssr_related) {
			temp_restart_level = dev->restart_level;
			pr_err("ADSP set_ssr_related,save temp_restart_level=%d\n", temp_restart_level);
	...
	if (strnstr(name, "adsp", strlen(name))) {
#ifdef ENABLE_ADSP_CRASH
		dev->restart_level = temp_restart_level;
#else
		if (set_ssr_related) {
			set_ssr_related = false;
			pr_err("ADSP restore restart_level=%d\n", temp_restart_level);
			dev->restart_level = temp_restart_level;
		}
#endif
	}
		...
		size_t cnt, loff_t *ppos)
{
	...
	if (!strcmp(subsys->desc->name, "adsp"))
...
struct subsys_device *subsys_register(struct subsys_desc *desc)
{
	...
	if (ofnode) {
		...
		subsys->restart_level = RESET_SUBSYS_COUPLED;
		if (is_debug_mode && !(force_ssr_related & (1 << subsys->id)))
