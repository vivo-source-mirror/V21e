...
static int qpnp_hap_parse_dt (struct qpnp_hap *hap)
{
	...
	hap->timeout_reverse = of_property_read_bool(pdev->dev.of_node, "vivo,timeout_reverse");
	printk(KERN_ERR "ccm_vibrator: timeout_reverse %d\n", hap->timeout_reverse);
	...
	if (hap->i0 < 0) {
		...
		/* use adec pin */
		rc = gpio_request(hap->adec, "vivo,motor-i0");
		if (rc < 0)
			printk(KERN_ERR "ccm_vibrator: request vivo,motor-i0 error: %d\n", rc);
		rc = gpio_direction_output(hap->i0, 0);
		if (rc < 0) {
			printk(KERN_ERR "%s Unable to set gpio %d, the rc is %d \n", __func__, hap->i0, rc);
			return rc;
		}
	...
	if (hap->timeout_reverse)
		rc = gpio_direction_output(hap->timeout, 1);
	else
		rc = gpio_direction_output(hap->timeout, 0);
#ifdef GPIO_VALUE_DEBUG
	printk(KERN_INFO "ccm_vibrator %s hap->timeout: %d rc %d \n", __func__, gpio_get_value(hap->timeout), rc);
#endif
	if (rc < 0) {
...
static void stop_motor_work(struct qpnp_hap *hap)
{
	...
	if (rc < 0) {
		printk(KERN_ERR"ccm_vibrator: pwm_apply_state: rc %d\n", rc);
	}
#else
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	motor_period = (int)period;
	printk(KERN_ERR"ccm_vibrator: motor period is %d\n", motor_period);
