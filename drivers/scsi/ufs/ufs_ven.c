...
void ufshcd_io_stats(struct ufs_hba *hba, struct ufshcd_lrb *lrbp, unsigned int tag)
{
	...
	if (ktime_to_ms(delta) > UFS_STAT_LATENCY_MAX_TIME_MS) {
		hba->io_stats.stat[gear][size][cmd_type].lat_abn_cnt++;
		pr_err("Warning: ufs_io latency %lldus abn_cnt %u cmd %#x lba %#llx sz %#x db %#x lun %d tag %u send_t %lld cmp_t %lld gear %d\n",
				ktime_to_us(delta),
				hba->io_stats.stat[gear][size][cmd_type].lat_abn_cnt,
				cmd_op,
				(unsigned long long)blk_rq_pos(rq),
...
show_io_sum(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (k = 0; k < CMD_TYPE_MAX; k++)
		for (j = 0; j < UFS_STAT_SIZE_MAX; j++) {
			...
			s64 lat_max = 0;
			u_int64_t size = 0;
			u_int32_t lat_abn_cnt = 0;
			u_int64_t stat_io_cnt;
			io_stats_info_t *stat;
			int len = SIZE_PAD_LENGTH;
			...
			for (i = 0; i < UFS_STAT_GEAR_MAX; i++) {
				...
				lat_abn_cnt += stat->lat_abn_cnt;
			}

			spin_unlock_irqrestore(&hba->io_stats.lock, flags);
			total_lat_abn_cnt += lat_abn_cnt;
			if ((k <= CMD_WRITE) /* read or write */
					|| (((k == CMD_DISCARD) || (k == CMD_FLUSH)) && (j == UFS_STAT_SIZE_OTHER))) {
				seq_printf(&m,
						"%s_%s_%-*s: %lld\n",
						ufs_support_size_str[j],
						ufs_support_cmd_type_str[k],
...
__show_nand_info(struct ufs_hba *hba, char *buf, int format)
{
	...
	for (i = 0; i < ARRAY_SIZE(hba->sdev_ufs_lu); i++) {
		sdp = hba->sdev_ufs_lu[i];
		if (sdp) {
	...
	pm_runtime_get_sync(hba->dev);
	mutex_lock(&hba->sdev_ufs_rpmb->state_mutex);
	if (hba->sdev_ufs_rpmb->sdev_state != SDEV_RUNNING) {
	...
	mutex_unlock(&hba->sdev_ufs_rpmb->state_mutex);
	...
	if (avg_erase && nif->write_size && sdkp->capacity) {
		if (nif->model[4] == '7' //if the product is SATURN controller
			...
			//convert 4KB unit to 100MB unit
			nif->wai = (((avg_erase + 1) * (uint64_t)sdkp->capacity) * 4 * KB / MB) * 100
				/ (nif->write_size * 10);
			nif->write_size /= 10;
		}
	...
	for (j = 0; j < UFS_STAT_SIZE_MAX; j++)
		for (k = 0; k < CMD_TYPE_MAX; k++) {
			...
			for (i = 0; i < UFS_STAT_GEAR_MAX; i++) {
				...
				io_cnt += stat->io_cnt;
				lat_max = max(ktime_to_us(stat->lat_max), lat_max);
				total_lat_abn_cnt += stat->lat_abn_cnt;
			}
			spin_unlock_irqrestore(&hba->io_stats.lock, flags);
	...
	if (format != UFS_NAND_INFO_OUTPUT_HEX_CLOUD) {
		...
		seq_printf(&m, "lat_abn_cnt    : %u\n", total_lat_abn_cnt);
	}

	if (format == UFS_NAND_INFO_OUTPUT_HEX_CLOUD)
		seq_printf(&m, "warn_io_cnt    : %u\n", total_lat_abn_cnt);
...
{
	if (sdev->lun < ARRAY_SIZE(hba->sdev_ufs_lu))
...
ufshcd_product_name_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (hba->ufs_vinfo.product_name[0]) {
		return snprintf(buf,
				hba->ufs_vinfo.product_name[QUERY_DESC_LENGTH_OFFSET],
				"%.16s\n", hba->ufs_vinfo.product_name+QUERY_DESC_HDR_SIZE);
	}
	...
	err = ufshcd_read_string_desc(hba,
			...
			hba->ufs_vinfo.product_name, sizeof(hba->ufs_vinfo.product_name),
			ASCII_STD);
	pm_runtime_put_sync(hba->dev);

	err = snprintf(buf, hba->ufs_vinfo.product_name[QUERY_DESC_LENGTH_OFFSET],
			"%.16s\n", hba->ufs_vinfo.product_name+QUERY_DESC_HDR_SIZE);
