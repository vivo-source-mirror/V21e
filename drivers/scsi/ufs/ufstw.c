...
static inline void ufstw_lu_update(struct ufstw_lu *tw)
{
	...
	if (ret)
		pr_err("ufs TW_enable failed: %d", ret);

	ret = ufstw_set_lu_flag(tw, QUERY_FLAG_IDN_TW_FLUSH_DURING_HIBERN,
			&tw->tw_flush_during_hibern_enter);
	if (ret)
		pr_err("ufs set tw_flush_during_hibern_enter failed: %d", ret);
