			     ...
			     sector_t lba, int transfer_len)
{
	...
	if (hba->io_stats.log_enable == UFS_VENDOR_TRACE_LOG_PRINT_ENABLE)
		pr_err("ufs_io %s: %s: seq_no=%u lun=0x%x cmd_id=0x%02x lba=0x%llx txfer_len=%d"
			" tag=%u, doorbell=0x%x outstanding=0x%x idn=%d time=%lld us\n",
		entry->cmd_type,
		entry->str,
		entry->seq_num,
		entry->lun,
					...
					unsigned int tag, const char *str)
{
	...
	if (lrbp->cmd) { /* data phase exists */
		...
		if ((opcode == READ_10) || (opcode == WRITE_10) || (opcode == UNMAP)) {
			/*
...
static int ufshcd_queuecommand(struct Scsi_Host *host, struct scsi_cmnd *cmd)
{
	...
	if (add_tag == -EAGAIN) {
		...
		err = SCSI_MLQUEUE_HOST_BUSY;
		hba->ufs_stats.scsi_blk_reqs.ts = ktime_get();
		hba->ufs_stats.scsi_blk_reqs.busy_ctx = UFSHCD_HPB;
		ufshcd_release_all(hba);
		goto out;
	}
	...
	case OCS_SUCCESS:
		...
		case UPIU_TRANSACTION_RESPONSE:
			...
			if (!hba->pm_op_in_progress &&
				...
				hba->clk_gating.state == CLKS_ON &&
				hba->vreg_info.vcc->enabled == TRUE &&
				hba->vreg_info.vdd_hba->enabled == TRUE) {
				/*
...
static void ufshcd_exception_event_handler(struct work_struct *work)
{
	...
	if (ufshcd_is_shutdown_ongoing(hba)) {
		dev_err(hba->dev, "%s: Return with ufshcd_is_shutdown_ongoing.\n",
				__func__);
		goto out;
	}
...
static int ufshcd_ioctl(struct scsi_device *dev, int cmd, void __user *buffer)
{
	...
	case UFS_IOCTL_QUERY:
		...
		if (down_read_trylock(&hba->ioctl_lock)) {
			...
			pr_err("[DEBUG]%s: shutdown on-going, do nothing, bails out with error\n", __func__);
			err = -EBUSY;
		}
		ufshcd_release_all(hba);
		pm_runtime_put_sync(hba->dev);
