...
static ssize_t fp_id_object_show(struct kobject *k, struct attribute *attr, char *buf)
{
	...
	pr_info("fp_project_name:%s, get_fp_id=%d, fp_frame_id=%s, fp_up=%d, fp_down=%d, fp_gpio=%d.\n", fp_project_name, get_fp_id(), fp_frame_id, fp_up, fp_down, fp_gpio);
	pr_info("%s: count_reset_high_ground=%d, count_reset_high_suspend=%d, count_reset_low_ground=%d, count_reset_low_suspend=%d.\n", __func__, count_reset_high_ground, count_reset_high_suspend, count_reset_low_ground, count_reset_low_suspend);
...
fp_id_probe(struct platform_device *pdev)
{
	...
	if ((!strncmp(fp_project_name, "PD2067F_EX", 10)) || (!strncmp(fp_project_name, "PD2107F_EX", 10))) {
		fp_id = GOODIX_GF9578;
		pr_info("%s: return gf9578  directly\n", __func__);
		return 0;
	}

	if ((!strncmp(fp_project_name, "PD1948F_EX", 10)) || (!strncmp(fp_project_name, "PD1931", 6)) || (!strncmp(fp_project_name, "PD1832F_EX", 10))) {
		fp_id = GOODIX_GF9518;
		pr_info("%s: return gf9518  directly\n", __func__);
		return 0;
	}

	if ((!strncmp(fp_project_name, "PD2039F_EX", 10)) || (!strncmp(fp_project_name, "PD2006F_EX", 10))) {
		fp_id = GOODIX_GF9518;
		pr_info("%s: return gf9518  directly\n", __func__);
		return 0;
	}

	if ((!strncmp(fp_project_name, "PD1923", 6)) || (!strncmp(fp_project_name, "PD1824", 6)) || (!strncmp(fp_project_name, "PD1936", 6))) {
		fp_id = GOODIX_GF9518;
		pr_info("%s: return gf9518  directly\n", __func__);
		return 0;
	}

	if ((!strncmp(fp_project_name, "PD1916", 6)) || (!strncmp(fp_project_name, "PD1922", 6))) {
		fp_id = GOODIX_GF9518;
		pr_info("%s: return gf9518  directly\n", __func__);
		return 0;
	}
	...
	pr_info("%s: set fp-id pull up,get gpio value = %d\n", __func__, fp_up);
	ret = select_pin_ctl(&pdev->dev, "fp_id_gpio_down");
	mdelay(5);
	if (ret)
		return -ERRORFP;
	fp_down = gpio_get_value(fp_gpio);
	pr_info("%s: set fp-id pull down,get gpio value = %d\n", __func__, fp_down);
	if (!strncmp(fp_project_name, "PD1806", 6)) {
		if (fp_up == 0) {
			fp_id = GOODIX_GF5288;
			pr_info("%s: gpio value: %d, return gf5288 ", __func__, fp_up);
		} else {
			fp_id = GOODIX_GF9518;
			pr_info("%s: gpio value: %d, return gf9518 ", __func__, fp_up);
		}
		return 0;
