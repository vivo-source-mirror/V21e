...
static const struct adc_channels adc_chans_pmic5[ADC_MAX_CHANNEL] = {
	...
	[ADC_BAT_ID_PU2]	= ADC_CHAN_TEMP("bat_id", 1,
					SCALE_HW_CALIB_DEFAULT)
	/* vivo use this channel for primary board temp */
	[ADC_AMUX_THM1_PU2]	= ADC_CHAN_TEMP("amux_thm1_pu2", 1,
					SCALE_HW_CALIB_THERM_100K_PULLUP)
	/* vivo use this channel for parallel board temp */
	...
	/*[ADC_GPIO3_PU2]	= ADC_CHAN_TEMP("gpio3_pu2", 1,
					SCALE_HW_CALIB_THERM_100K_PULLUP)*//*vivo delete*/
	[ADC_GPIO4_PU2]	= ADC_CHAN_TEMP("gpio4_pu2", 1,
					SCALE_HW_CALIB_THERM_100K_PULLUP)
	[ADC_GPIO2] = ADC_CHAN_VOLT("gpio8_vadc", 1,
					SCALE_HW_CALIB_DEFAULT)
	/*vivo add*/
	[ADC_GPIO4] = ADC_CHAN_VOLT("batid_vadc", 1,
					SCALE_HW_CALIB_DEFAULT)
	[ADC_GPIO3_PU2] = ADC_CHAN_TEMP("pl_ntc_adc", 1,
					SCALE_HW_CALIB_THERM_100K_PULLUP)
	/*vivo end*/
	/*VIVO add*/
	[ADC_GPIO1]	= ADC_CHAN_VOLT("batid_vadc2", 1,
					SCALE_HW_CALIB_DEFAULT)
};
/*VIVO add below, please keep same with adc_chans_pmic5[] && change your owner chan..*/
static const struct adc_channels adc_chans_pmic5_l[ADC_MAX_CHANNEL] = {
	[ADC_REF_GND]		= ADC_CHAN_VOLT("ref_gnd", 1,
					SCALE_HW_CALIB_DEFAULT)
	[ADC_1P25VREF]		= ADC_CHAN_VOLT("vref_1p25", 1,
					SCALE_HW_CALIB_DEFAULT)
...
const struct adc_data data_pmic5_l = {
	.full_scale_code_volt = 0x70e4,
	/* On PM8150B, IBAT LSB = 10A/32767 */
	.full_scale_code_cur = 10000,
	.adc_chans = adc_chans_pmic5_l,
	.decimation = (unsigned int []) {250, 420, 840},
	.hw_settle = (unsigned int []) {15, 100, 200, 300, 400, 500, 600, 700,
					800, 900, 1, 2, 4, 6, 8, 10},
};
...
static const struct of_device_id adc_match_table[] = {
	...
	{
		/*  VIVO_CHG: request from #case 03818661 for :
		 *		ADC1: PM8150L_AMUX_1 (100k pull up) --> Chan=0x4D, use for "ADC_CHAN_VOLT".
		 *		ADC2: PM8150__AMUX_1 (100k pull up) --> Chan=0x4D, use for "ADC_CHAN_TEMP".
         */
		.compatible = "qcom,spmi-adc5-l",
		.data = &data_pmic5_l,
	},
	{
