...
#else
	return request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				waveform_rtp_table[aw8697->rtp_file_num].rtp_name, aw8697->dev, GFP_KERNEL,
				aw8697, aw8697_rtp_loaded);
...
static void aw8697_rtp_work_routine(struct work_struct *work)
{
	...
	/* fw loaded */
	ret = request_firmware(&rtp_file,
			waveform_rtp_table[aw8697->rtp_file_num].rtp_name,
			aw8697->dev);
	if (ret < 0) {
		pr_err("%s: failed to read %s\n", __func__,
				waveform_rtp_table[aw8697->rtp_file_num].rtp_name);
...
static ssize_t aw8697_cali_show(struct device *dev, struct device_attribute *attr, //key
		char *buf)
{
#ifdef TIMED_OUTPUT
	struct timed_output_dev *to_dev = dev_get_drvdata(dev);
	...
	aw8697_haptic_get_f0(aw8697);
	mutex_unlock(&aw8697->lock);
	return snprintf(buf, PAGE_SIZE, "%s f0 %d.%d (range:%d.%d-%d.%d)hz f0_offset=%d\n",
		(aw8697->f0 >= aw8697->freq_min && aw8697->f0 <= aw8697->freq_max) ? "ok" : "fail",
		aw8697->f0/10, aw8697->f0%10,
		aw8697->freq_min/10, aw8697->freq_min%10, aw8697->freq_max/10, aw8697->freq_max%10, atomic_read(&aw8697->f0_freq_cali));
}
...
//	wake_lock_init(&aw8697->wake_lock, WAKE_LOCK_SUSPEND, "vivo-aw8697-wakelock");
	wakeup_source_init(&aw8697->ws, "vivo-aw8697-wakelock");
		...
		struct ff_effect *effect, struct ff_effect *old)
{
	...
	case FF_CONSTANT:
		...
		tmp = level * AW8697_HAPTIC_RATED_VOLTAGE;
	...
	case FF_PERIODIC:
		...
		data[CUSTOM_DATA_TIMEOUT_MSEC_IDX] = (play->times_us % USEC_PER_SEC) / USEC_PER_MSEC;
