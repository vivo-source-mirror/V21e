...
/* add by daizhihui for vivo_pem start */
//git policy
int pem_setfrep(unsigned int cpu, unsigned int max, unsigned int min)
{
	struct cpufreq_policy *policy = cpufreq_cpu_get(cpu);
	struct cpufreq_policy new_policy;
	int ret;
	...
	if (!ret) {
		...
		pr_info(" pem_driver: pem_setfrep fail, cpu(%d) min(%d) max(%d) src_min(%d) src_max(%d)!\n",
				cpu, min, max, policy->min, policy->max);
	}
...
EXPORT_SYMBOL_GPL(pem_setfrep);
/* add by daizhihui for vivo_pem end */
