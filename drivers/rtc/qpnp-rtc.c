			...
			u16 base, int count)
{
	...
	int i;
	for (i = 0; i < count; i++) {
		printk("rtc-qpnp_write [0x%x]=0x%x", base+i, rtc_val[i]);
	}
#endif
...
#ifdef CONFIG_VIVO_LINUXBSP_EDIT
	dev_err(dev, "%s|%d|%d Alarm Set for h:r:s=%d:%d:%d, d/m/y=%d/%d/%d\n",
...
#ifdef CONFIG_VIVO_LINUXBSP_EDIT
	dev_err(dev, "%s|%d|%d Alarm read for - h:r:s=%d:%d:%d, d/m/y=%d/%d/%d\n",
		current->comm, current->pid, current->tgid,
		alarm->time.tm_hour, alarm->time.tm_min,
				alarm->time.tm_sec, alarm->time.tm_mday,
				alarm->time.tm_mon, alarm->time.tm_year);
