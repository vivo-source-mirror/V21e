...
module_param(sdx50_timeout, int, S_IRUGO);
module_param(sdx50_boot_fail, int, S_IRUGO);
...
void set_boot_fail_action(atomic_t *p, u32 action)
{
	if (download_mode) {
		atomic_set(p, action);
	} else {
		atomic_set(p, BOOT_FAIL_ACTION_COLD_RESET);
	}
}
int esoc_set_boot_fail_action(struct esoc_clink *esoc_clink, u32 action)
{
	...
	set_boot_fail_action(&mdm_drv->boot_fail_action, action);
...
static int mdm_handle_boot_fail(struct esoc_clink *esoc_clink, u8 *pon_trial)
{
	...
	if (sdx50_boot_fail ==  0) {
		sdx50_boot_fail = 1;
		if (sdx50_timeout) {
			self_data = "sdx50_timeout";
		} else {
			self_data = "sdx50_boot_fail";
		}
		snprintf(big_data, 380, "[{\"moduleid\":\"3200\",\"eventId\":\"00058|012\",\"times\":\"1\",\"extype\":\"1\",\"exInfo\":\"%s\"}]", self_data);
		writeData("cds_3200", NULL, big_data);
	}
...
/*begin: add by yangjianhua for 5G*/
extern unsigned int is_5g_ap_board;
extern unsigned int is_atCmd;
/*end: add by yangjianhua for 5G*/
static int mdm_subsys_powerup(const struct subsys_desc *crashed_subsys)
{
	...
	if (1 == is_atCmd) {
	    if (1 == is_5g_ap_board) {
			esoc_mdm_log("not Powerup mdm subsys\n");
			return -EIO;
	    }
	}
	/*end: add by yangjianhua for 5G*/
	do {
		 ...
		 * case.
		 */
		if (esoc_clink->auto_boot || !download_mode)
			timeout = 30 * HZ;
		esoc_mdm_log(
...
int esoc_ssr_probe(struct esoc_clink *esoc_clink, struct esoc_drv *drv)
{
	...
	set_boot_fail_action(&mdm_drv->boot_fail_action, BOOT_FAIL_ACTION_DEF);
