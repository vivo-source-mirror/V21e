...
#define pr_fmt(fmt)	"vibrator: %s: " fmt, __func__
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&chip->lock);
	pr_err("qpnp_vib_store_duration duration %d\n", val);//Added by vivo-sensor-team
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	chip->state = val;
	/* Modified by vivo-sensor-team, begin*/
	pr_err("active state = %d, time = %llums, pid is %d\n", chip->state, chip->vib_play_ms, current->pid);
	/* Modified by vivo-sensor-team, end*/
	mutex_unlock(&chip->lock);
	schedule_work(&chip->vib_work);
