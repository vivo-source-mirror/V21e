...
/* rongqianfeng add begin */
extern unsigned int bsp_test_mode;
/* rongqianfeng add end */
...
static unsigned long lowmem_scan(struct shrinker *s, struct shrink_control *sc)
{
	...
	for_each_process(tsk) {
		...
		if (unlikely(bsp_test_mode)) {
			if (!strncmp(tsk->comm, "om.vivo.bsptest", strlen("om.vivo.bsptest")) ||
				    !strncmp(tsk->comm, "stressapptest", strlen("stressapptest"))) {
