			...
			unsigned char *dest_buf, int dest_len, int pid)
{
	...
	//vivo liangxin add for QRCT test enter begin
	mask_len = (req->num_bits - 1) / 8 + 1;
	//mask_len = EVENT_COUNT_TO_BYTES(req->num_bits - 1);
	pr_err("diag: In %s, invalid event mask len: %d %d \n", __func__,
mask_len, event_mask.mask_len);
	//vivo liangxin add for QRCT test enter end
