...
/* luzhi add begin for frame sync */
static struct kobject timestamp_kobject;
static int64_t main_to_main_timedelta;
static int64_t aux_to_aux_timedelta;
static int64_t main_aux_timedelta;
static int32_t first_sof_come_sensor;
static int32_t master_core_index;
/* luzhi add end for frame sync */
...
start_only:
	...
	/* luzhi add for frame sync*/
	first_sof_come_sensor = 0;
	...
	void                              *payload)
{
	...
	uint32_t  sof_status = 0;
	bool sof_sent = false;
	static int64_t main_timestamp_temp;
	static int64_t aux_timestamp_temp;
	static uint64_t main_timestamp;
	static uint64_t aux_timestamp;
	...
	list_for_each_entry(ife_src_res,
		...
		case CAM_ISP_HW_VFE_IN_CAMIF:
			...
			if (!sof_status && !sof_sent) {
				...
				CAM_DBG(CAM_ISP, "ctx_index=%d  core_index=%d sof_status = %d timestamp=%lld boot_timestamp=%lld",
					ife_hw_mgr_ctx->ctx_index, evt_payload->core_index, sof_status, sof_done_event_data.timestamp, sof_done_event_data.boot_time);

				/*luzhi add begin for hardware sync timestamp calculate*/
				if (first_sof_come_sensor == 0) {
					first_sof_come_sensor = 1;
					master_core_index = evt_payload->core_index;
				}

				if (evt_payload->core_index == master_core_index) {
					main_timestamp = sof_done_event_data.timestamp/1000;
					main_to_main_timedelta = (int64_t)(main_timestamp - main_timestamp_temp);
					CAM_DBG(CAM_ISP, "main_timestamp=%llu main_timestamp_temp=%llu", main_timestamp, main_timestamp_temp);
					main_timestamp_temp = main_timestamp;
				} else {
					aux_timestamp = sof_done_event_data.timestamp/1000;
					main_aux_timedelta = (int64_t)(aux_timestamp - main_timestamp);
					aux_to_aux_timedelta = (int64_t)(aux_timestamp - aux_timestamp_temp);
					CAM_DBG(CAM_ISP, "main_aux_timedelta=%lld main_to_main_timedelta=%lld aux_to_aux_timedelta=%lld aux_timestamp=%llu aux_timestamp_temp=%llu",
						main_aux_timedelta, main_to_main_timedelta, aux_to_aux_timedelta, aux_timestamp, aux_timestamp_temp);
					aux_timestamp_temp = aux_timestamp;
				}
				/*luzhi add end for hardware sync timestamp calculate*/
			}
			break;
		case CAM_ISP_HW_VFE_IN_CAMIF_LITE:
			break;
...
/*luzhi add begin for hardware sync timestamp calculate */
static ssize_t  main_to_main_timedelta_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, 8, "%lld", main_to_main_timedelta);
}

static ssize_t  aux_to_aux_timedelta_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, 8, "%lld", aux_to_aux_timedelta);
}

static ssize_t  main_aux_timedelta_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, 8, "%lld", main_aux_timedelta);
}

static ssize_t timestamp_object_show(struct kobject *k, struct attribute *attr, char *buf)
{
...
static ssize_t timestamp_object_store(struct kobject *k, struct attribute *attr, const char *buf, size_t size)
{
	struct kobj_attribute *kobj_attr;
	int ret = -EIO;
...
static struct kobj_attribute main_to_main_timeDelta_attribute =
__ATTR(main_to_main_timeDelta, 0644, main_to_main_timedelta_show, NULL);

static struct kobj_attribute aux_to_aux_timeDelta_attribute =
__ATTR(aux_to_aux_timeDelta, 0644, aux_to_aux_timedelta_show, NULL);

static struct kobj_attribute main_aux_timeDelta_attribute =
__ATTR(main_aux_timeDelta, 0644, main_aux_timedelta_show, NULL);

static const struct sysfs_ops timestamp_sysfs_ops = {
	.show = timestamp_object_show,
	.store = timestamp_object_store,
};

static struct kobj_type timestamp_object_type = {
	.sysfs_ops = &timestamp_sysfs_ops,
	.release	= NULL,
};

static void timestamp_kobj_init(void)
{
	int rc = 0;
	memset(&timestamp_kobject, 0x00, sizeof(timestamp_kobject));

	if (kobject_init_and_add(&timestamp_kobject, &timestamp_object_type, NULL, "timestamp")) {
		kobject_put(&timestamp_kobject);
		return;
	}

	kobject_uevent(&timestamp_kobject, KOBJ_ADD);
	rc = sysfs_create_file(&timestamp_kobject, &main_to_main_timeDelta_attribute.attr);
	rc = sysfs_create_file(&timestamp_kobject, &aux_to_aux_timeDelta_attribute.attr);
	rc = sysfs_create_file(&timestamp_kobject, &main_aux_timeDelta_attribute.attr);
}
/*luzhi add end for hardware sync timestamp calculate */
