		...
		uint32_t size)
{
	...
	/*qcom patch modify memory alloc to avoid tof alloc mem failed of low probability
	http://smartgit/gerrit/#/c/1392863 */
	tmp = (struct i2c_settings_list *)
		kvzalloc(sizeof(struct i2c_settings_list), GFP_KERNEL); /*qcom patch modify*/
	...
	tmp->i2c_settings.reg_setting = (struct cam_sensor_i2c_reg_array *)
		kvzalloc(size * sizeof(struct cam_sensor_i2c_reg_array), /*qcom patch*/
			GFP_KERNEL);
	if (tmp->i2c_settings.reg_setting == NULL) {
		list_del(&(tmp->list));
		kvfree(tmp); /*qcom patch*/
		return NULL;
	}
...
int32_t delete_request(struct i2c_settings_array *i2c_array)
{
	...
	list_for_each_entry_safe(i2c_list, i2c_next,
		...
		kvfree(i2c_list->i2c_settings.reg_setting); /*qcom patch*/
		list_del(&(i2c_list->list));
		kvfree(i2c_list); /*qcom patch*/
	}
		...
		struct cam_hw_soc_info *soc_info, int gpio_en)
{
	...
	for (i = 0; i < size; i++) {
		CAM_ERR(CAM_SENSOR, "i: %d, gpio %d dir %ld",  i,
	...
	if (gpio_en) {
		for (i = 0; i < size; i++) {
			...
			if (rc) {
				...
				CAM_ERR(CAM_SENSOR, "gpio %d:%s request success",
					gpio_tbl[i].gpio, gpio_tbl[i].label);
			}
		}
	} else {
