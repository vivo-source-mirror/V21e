...
// add by yinpeng.zhang for sensor streamoff start
static int vivo_cam_streamoff_check(struct cam_sensor_ctrl_t *s_ctrl)
{
	int rc = 0;
	int index = 0;
	int read_count = 0;
	uint32_t read_data = 0;
	struct vivo_streamoff_id_reg {
		uint32_t sensor_id;
		uint32_t check_reg;
	};
	struct vivo_streamoff_id_reg streamoff_sensor_info[2] = {
		{0xf881, 0x0005}, //s5kgh1
		{0x0971, 0x0005}  //s5kgw1
		//......
	};

	for (index = 0; index < 2; index++) {
		if (s_ctrl->sensordata->slave_info.sensor_id == streamoff_sensor_info[index].sensor_id) {
			while ((read_count++) < 15) {
				rc = camera_io_dev_read(&(s_ctrl->io_master_info),  streamoff_sensor_info[index].check_reg,
											&read_data, s_ctrl->sensor_probe_addr_type,
											s_ctrl->sensor_probe_data_type);
				if (rc < 0) {
					CAM_ERR(CAM_SENSOR, "vivo cam read %04x 0x0005 failed", s_ctrl->sensordata->slave_info.sensor_id);
					return rc;
				}

				CAM_DBG(CAM_SENSOR, "vivo cam read addr 0x0005 : 0x%x, read_count [%d]", (read_data >> 8), read_count);
				if ((read_data >> 8) != 0xff) {
					rc = cam_sensor_apply_settings(s_ctrl, 0, CAM_SENSOR_PACKET_OPCODE_SENSOR_STREAMOFF);
					if (rc < 0) {
						CAM_ERR(CAM_SENSOR, "vivo cam cannot apply streamoff settings");
					}
					msleep(10);
				} else {
					CAM_INFO(CAM_SENSOR, "vivo cam %04x streamoff success", s_ctrl->sensordata->slave_info.sensor_id);
					return rc;
				}
			}
		}
...
// add by yinpeng.zhang for sensor streamoff end

/*luzhi add for sensor temperature read*/
static void cam_sensor_read_thermal(struct cam_sensor_ctrl_t *s_ctrl)
{
	uint32_t read_data = 0;
	static uint16_t read_count;
	uint16_t data_shift = s_ctrl->sensordata->slave_info.sensor_thermal_shift;
	uint16_t data_mask = s_ctrl->sensordata->slave_info.sensor_thermal_mask;
	read_count++;
	if (read_count > 60) {
		read_count = 0;
		if (s_ctrl->sensordata->slave_info.sensor_thermal_addr != 0x0000) {
			camera_io_dev_read(&(s_ctrl->io_master_info),
				s_ctrl->sensordata->slave_info.sensor_thermal_addr, &read_data,
				s_ctrl->sensor_probe_addr_type, s_ctrl->sensor_probe_data_type);
			if (read_data >= 0x8000 && read_data <= 0xffff) {
				CAM_INFO(CAM_SENSOR, "The sensor temperature is negative, not as a reference!");
				s_ctrl->sensordata->slave_info.sensor_temperature = 0;
			} else {
				s_ctrl->sensordata->slave_info.sensor_temperature = (read_data>>data_shift) & data_mask;
			}
			CAM_INFO(CAM_SENSOR, "sensor_thermal_addr:%d, sensor_probe_addr_type:%d, sensor_probe_data_type:%d, sensor_temperature:%d", 
				s_ctrl->sensordata->slave_info.sensor_thermal_addr, s_ctrl->sensor_probe_addr_type, s_ctrl->sensor_probe_data_type, s_ctrl->sensordata->slave_info.sensor_temperature);
		} else {
			s_ctrl->sensordata->slave_info.sensor_temperature = 0;
		}
	}
...
{
	int32_t rc = 0;
	uint16_t probe_flag, pipeline_delay;
	...
	probe_flag = (probe_info->reserved & 0xff00) >> 8;
	pipeline_delay = probe_info->reserved & 0x00ff;
	s_ctrl->pipeline_delay = pipeline_delay;
	s_ctrl->sensordata->slave_info.probe_flag = probe_flag;
	s_ctrl->sensor_probe_addr_type =  probe_info->addr_type;
	s_ctrl->sensor_probe_data_type =  probe_info->data_type;
	//vivo dongzhixiang add for dual sensorID and module comatible 2019/4/13 begin
	if (probe_flag > 0) {
		s_ctrl->sensordata->slave_info.extend_slave_addr1 = probe_info->extend_slave_addr1;
		s_ctrl->sensordata->slave_info.extend_addr1_type = probe_info->extend_addr1_type;
		s_ctrl->sensordata->slave_info.extend_data1_type = probe_info->extend_data1_type;
		s_ctrl->sensordata->slave_info.extend_reg_addr1 = probe_info->extend_reg_addr1;
		s_ctrl->sensordata->slave_info.extend_reg_data1 = probe_info->extend_reg_data1;
		s_ctrl->sensordata->slave_info.extend_data_mask1 = probe_info->extend_data_mask1;
		s_ctrl->sensordata->slave_info.extend_slave_addr2 = probe_info->extend_slave_addr2;
		s_ctrl->sensordata->slave_info.extend_addr2_type = probe_info->extend_addr2_type;
		s_ctrl->sensordata->slave_info.extend_data2_type = probe_info->extend_data2_type;
		s_ctrl->sensordata->slave_info.extend_reg_addr2 = probe_info->extend_reg_addr2;
		s_ctrl->sensordata->slave_info.extend_reg_data2 = probe_info->extend_reg_data2;
		s_ctrl->sensordata->slave_info.extend_data_mask2 = probe_info->extend_data_mask2;
		/*hujin add for thermal meter begin*/
		s_ctrl->sensordata->slave_info.sensor_thermal_addr = probe_info->thermal_addr;
		s_ctrl->sensordata->slave_info.sensor_thermal_mask = probe_info->thermal_mask;
		s_ctrl->sensordata->slave_info.sensor_thermal_shift = probe_info->thermal_shift;
		s_ctrl->sensor_probe_data_type =  probe_info->data_type;
		memcpy(s_ctrl->sensor_name, probe_info->sensor_name, MAX_SENSOR_NAME_SIZE);
		s_ctrl->sensor_name[MAX_SENSOR_NAME_SIZE - 1] = '\0';
		CAM_INFO(CAM_SENSOR, "sensor debug thermal %s", s_ctrl->sensor_name);
		/*hujin add for thermal meter end*/
	}
	//vivo dongzhixiang add for dual sensorID and module comatible 2019/4/13 end
	CAM_DBG(CAM_SENSOR,
...
static uint16_t vivo_cam_regdata_by_mask(uint32_t reg_data, int16_t reg_data_mask)
{
	uint16_t reg_data_t = (uint16_t)(reg_data & 0xFFFF);

	if (!reg_data_mask)
		reg_data_mask = ~reg_data_mask;

	reg_data_t &= reg_data_mask;
	reg_data_mask &= -reg_data_mask;
	reg_data_mask -= 1;
	while (reg_data_mask) {
		reg_data_mask >>= 1;
...
{
	int rc = 0;
	uint32_t chipid = 0, second_chipid = 0;
	uint32_t reg_data1 = 0, reg_data2 = 0;
	uint16_t sid_temp;
	...
	rc = camera_io_dev_read(
		...
		s_ctrl->sensor_probe_data_type);
	if (rc < 0) {
		CAM_ERR(CAM_SENSOR, "sesor id: %04x I2C communication failed", slave_info->sensor_id);
		return rc;
	}
	CAM_INFO(CAM_SENSOR, "read id: %04x expected id 0x%04x probe type = %d",
		chipid, slave_info->sensor_id, s_ctrl->sensordata->slave_info.probe_flag);
	switch (s_ctrl->sensordata->slave_info.probe_flag) {
	case COMPATIBLE_DEFAULT: {
		if (cam_sensor_id_by_mask(s_ctrl, chipid) != slave_info->sensor_id) {
			CAM_ERR(CAM_SENSOR, "chip id %04x does not match %04x",
					chipid, slave_info->sensor_id);
			return -ENODEV;
		}
		break;
	}
	case ONLY_DOUBLE_SENSORID: {
		if (cam_sensor_id_by_mask(s_ctrl, chipid) != slave_info->sensor_id) {
			rc = camera_io_dev_read(
				&(s_ctrl->io_master_info),
				slave_info->extend_reg_addr1,
				&second_chipid, slave_info->extend_addr1_type,
				slave_info->extend_data1_type);
			CAM_INFO(CAM_SENSOR, "read id1: 0x%04x expected id 0x%04x:",
				reg_data1, slave_info->extend_reg_data1);
			if (vivo_cam_regdata_by_mask(second_chipid,
				slave_info->extend_data_mask1) != slave_info->extend_reg_data1) {
				return -ENODEV;
			}
		}
		break;
	}
	case DOUBLE_SENSORID_AND_COMPATIBLE: {
		if (cam_sensor_id_by_mask(s_ctrl, chipid) != slave_info->sensor_id) {
			rc = camera_io_dev_read(
				&(s_ctrl->io_master_info),
				slave_info->extend_reg_addr1,
				&second_chipid, slave_info->extend_addr1_type,
				slave_info->extend_data1_type);
			CAM_INFO(CAM_SENSOR, "read second_chipid: 0x%04x expected id 0x%04x:",
				second_chipid, slave_info->extend_reg_data1);
			if (vivo_cam_regdata_by_mask(second_chipid,
				slave_info->extend_data_mask1) != slave_info->extend_reg_data1) {
				return -ENODEV;
			}
		...
		if (rc == 0) {
				sid_temp = s_ctrl->io_master_info.cci_client->sid;
				s_ctrl->io_master_info.cci_client->sid = slave_info->extend_slave_addr2 >> 1;
				rc = camera_io_dev_read(
					&(s_ctrl->io_master_info),
					slave_info->extend_reg_addr2,
					&reg_data2, slave_info->extend_addr2_type,
					slave_info->extend_data2_type);
				CAM_INFO(CAM_SENSOR, "read id2: 0x%04x expected id 0x%04x:",
					reg_data2, slave_info->extend_reg_data2);
				if (vivo_cam_regdata_by_mask(reg_data2, slave_info->extend_data_mask2) != 
					slave_info->extend_reg_data2) {
					return -ENODEV;
				}
				s_ctrl->io_master_info.cci_client->sid = sid_temp;
		}
		break;
	}
	case ONLY_COMPATIBLE_EXTEND1: {
		if (cam_sensor_id_by_mask(s_ctrl, chipid) != slave_info->sensor_id) {
			CAM_ERR(CAM_SENSOR, "chip id %04x does not match %04x",
				chipid, slave_info->sensor_id);
			return -ENODEV;
		...
		if (rc == 0) {
			sid_temp = s_ctrl->io_master_info.cci_client->sid;
			s_ctrl->io_master_info.cci_client->sid = slave_info->extend_slave_addr1 >> 1;
			rc = camera_io_dev_read(
				&(s_ctrl->io_master_info),
				slave_info->extend_reg_addr1,
				&reg_data1, slave_info->extend_addr1_type,
				slave_info->extend_data1_type);
			CAM_INFO(CAM_SENSOR, "read id1: 0x%04x expected id 0x%04x:",
				reg_data1, slave_info->extend_reg_data1);
			if (vivo_cam_regdata_by_mask(reg_data1, slave_info->extend_data_mask1) != 
				slave_info->extend_reg_data1) {
				return -ENODEV;
			}
			s_ctrl->io_master_info.cci_client->sid = sid_temp;
		}
	...
	case BOTH_FOR_COMPTIBLE: {
		if (cam_sensor_id_by_mask(s_ctrl, chipid) != slave_info->sensor_id) {
			CAM_INFO(CAM_SENSOR, "chip id %04x does not match %04x",
				chipid, slave_info->sensor_id);
			return -ENODEV;
		}
		if (rc == 0) {
			...
			CAM_ERR(CAM_SENSOR, "read id1: 0x%04x failed", slave_info->extend_reg_addr1);
			return rc;
		}
		sid_temp = s_ctrl->io_master_info.cci_client->sid;
		s_ctrl->io_master_info.cci_client->sid = slave_info->extend_slave_addr2 >> 1;
		rc = camera_io_dev_read(
			...
			&reg_data2, slave_info->extend_addr2_type,
			slave_info->extend_data2_type);
		CAM_INFO(CAM_SENSOR, "read id2: 0x%04x expected id2 0x%04x:",
			reg_data2, slave_info->extend_reg_data2);
		if (vivo_cam_regdata_by_mask(reg_data2, slave_info->extend_data_mask2) != 
			slave_info->extend_reg_data2) {
			return -ENODEV;
...
static void cam_sensor_config_hiz(struct cam_sensor_ctrl_t *s_ctrl)
{
	uint16_t temp_addr = 0;
	int rc = 0;
	temp_addr = s_ctrl->io_master_info.cci_client->sid;
	s_ctrl->io_master_info.cci_client->sid = s_ctrl->sensordata->hiz_config.i2c_addr >> 1;
	rc = camera_io_dev_write(&(s_ctrl->io_master_info), &(s_ctrl->sensordata->hiz_config.reg_setting));
	if (rc < 0)
		CAM_ERR(CAM_SENSOR, "failed set hiz: %d", rc);
	else
		CAM_INFO(CAM_SENSOR, "success set hiz");
	s_ctrl->io_master_info.cci_client->sid = temp_addr;
}
	...
	void *arg)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
		...
		for (i = 0; i < 3; i++) { //vivo dongzhixiang add to reprobe if for cci error 2019/4/11
			...
			if (rc < 0) {
				cam_sensor_power_down(s_ctrl);
				msleep(20);
				CAM_INFO(CAM_SENSOR, "reprobe i = %d", i);
				if (i == 2 || rc != -EINVAL)
					goto free_power_settings;
			} else {
				break;
	...
	case CAM_ACQUIRE_DEV: {
		...
		if ((s_ctrl->is_probe_succeed == 0) ||
			(s_ctrl->sensor_state != CAM_SENSOR_INIT)) {
			CAM_ERR(CAM_SENSOR,
				"Not in right state to aquire %d sensor_id:0x%x,sensor_slave_addr:0x%x",
				s_ctrl->sensor_state,
				s_ctrl->sensordata->slave_info.sensor_id,
				s_ctrl->sensordata->slave_info.sensor_slave_addr);
			rc = -EINVAL;
		...
		if (s_ctrl->bridge_intf.device_hdl != -1) {
			CAM_ERR(CAM_SENSOR,
				"Device is already acquired sensor_id:0x%x,sensor_slave_addr:0x%x",
				s_ctrl->sensor_state,
				s_ctrl->sensordata->slave_info.sensor_id,
				s_ctrl->sensordata->slave_info.sensor_slave_addr);
			rc = -EINVAL;
	...
	case CAM_RELEASE_DEV: {
		if ((s_ctrl->sensor_state == CAM_SENSOR_INIT) ||
			...
			rc = -EINVAL;
			CAM_ERR(CAM_SENSOR,
				"Not in right state to release %d sensor_id:0x%x,sensor_slave_addr:0x%x",
				s_ctrl->sensor_state,
				s_ctrl->sensordata->slave_info.sensor_id,
				s_ctrl->sensordata->slave_info.sensor_slave_addr);
			goto release_mutex;
	...
	case CAM_START_DEV: {
		if ((s_ctrl->sensor_state == CAM_SENSOR_INIT) ||
			...
			rc = -EINVAL;
			CAM_ERR(CAM_SENSOR,
				"Not in right state to start %d sensor_id:0x%x,sensor_slave_addr:0x%x",
				s_ctrl->sensor_state,
				s_ctrl->sensordata->slave_info.sensor_id,
				s_ctrl->sensordata->slave_info.sensor_slave_addr);
			goto release_mutex;
	...
	case CAM_STOP_DEV: {
		if (s_ctrl->sensor_state != CAM_SENSOR_START) {
			rc = -EINVAL;
			CAM_ERR(CAM_SENSOR,
				"Not in right state to stop %d sensor_id:0x%x,sensor_slave_addr:0x%x",
				s_ctrl->sensor_state,
				s_ctrl->sensordata->slave_info.sensor_id,
				s_ctrl->sensordata->slave_info.sensor_slave_addr);
			goto release_mutex;
		...
		//add by zhangyinpeng for check streamoff start
		rc = vivo_cam_streamoff_check(s_ctrl);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "cannot apply streamoff settings");
		}
		//add by zhangyinpeng for check streamoff end
	...
	case CAM_CONFIG_DEV: {
		...
		/*luzhi add for sensor temperature read*/
		if (s_ctrl->i2c_data.init_settings.request_id != 0 &&
			s_ctrl->sensordata->slave_info.sensor_thermal_addr > 0)
			cam_sensor_read_thermal(s_ctrl);
		...
		if (s_ctrl->i2c_data.init_settings.is_settings_valid &&
			...
			CAM_INFO(CAM_SENSOR,
				"CAM_SENSOR_CONFIG Success init setting, sensor_id:0x%x,sensor_slave_addr:0x%x",
				s_ctrl->sensordata->slave_info.sensor_id,
				s_ctrl->sensordata->slave_info.sensor_slave_addr);
		}
		...
		if (s_ctrl->i2c_data.config_settings.is_settings_valid &&
			...
			CAM_INFO(CAM_SENSOR,
				"CAM_SENSOR_CONFIG Success mode setting, sensor_id:0x%x,sensor_slave_addr:0x%x",
				s_ctrl->sensordata->slave_info.sensor_id,
				s_ctrl->sensordata->slave_info.sensor_slave_addr);
...
int cam_sensor_power_up(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	//vivo dongzhixiang add to optimize power consumption 2019/5/31 begin
	if (s_ctrl->cam_vdd_regulator != NULL) {
		regulator_set_voltage(s_ctrl->cam_vdd_regulator,
			s_ctrl->cam_vdd_max_voltage, s_ctrl->cam_vdd_max_voltage);
	}
	//vivo dongzhixiang add to optimize power consumption 2019/5/31 end
	...
	if (rc < 0) {
		CAM_ERR(CAM_SENSOR, "power up the core is failed:%d", rc);
			...
			if (s_ctrl->cam_vdd_regulator != NULL) {
				regulator_set_voltage(s_ctrl->cam_vdd_regulator,
					s_ctrl->cam_vdd_min_voltage, s_ctrl->cam_vdd_max_voltage);
		    }
			//vivo dongzhixiang add to optimize power consumption 2019/5/31 end
		return rc;
	}
	...
	if (s_ctrl->sensordata->hiz_config.reg_setting.size > 0)
		cam_sensor_config_hiz(s_ctrl);
