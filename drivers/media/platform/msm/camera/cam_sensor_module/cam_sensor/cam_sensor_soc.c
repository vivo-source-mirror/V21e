...
static int32_t cam_sensor_driver_get_dt_data(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	struct device_node *src_node = NULL;
	uint32_t hiz_config[4];
	uint32_t temp;
	int num_of_hiz_regs, count_of_hiz_regs = 0;
	...
	src_node = of_parse_phandle(of_node, "cam_vdd-supply", 0);
	if (src_node != NULL) {
		if (of_property_read_u32(of_node, "cam_vdd-min-voltage",
		&s_ctrl->cam_vdd_min_voltage) >= 0 &&
		of_property_read_u32(of_node, "cam_vdd-max-voltage",
		&s_ctrl->cam_vdd_max_voltage) >= 0) {
			s_ctrl->cam_vdd_regulator = regulator_get(soc_info->dev, "cam_vdd");
			if (!s_ctrl->cam_vdd_regulator || IS_ERR(s_ctrl->cam_vdd_regulator)) {
				s_ctrl->cam_vdd_regulator = NULL;
			}
		}
	}
	//vivo dongzhixiang add to optimize power consumption 2019/5/31 end
	src_node = of_parse_phandle(of_node, "hiz-config", 0);
	if (src_node != NULL) {
		rc = of_property_read_u32_array(of_node, "hiz-config",
			hiz_config, 4);
		if (rc < 0) {
			CAM_INFO(CAM_SENSOR, "Invalid hiz-config");
		} else {
			num_of_hiz_regs = of_property_count_u32_elems(of_node, "hiz-regs");
			if (num_of_hiz_regs % 3 == 0 && num_of_hiz_regs > 3) {
				count_of_hiz_regs = num_of_hiz_regs / 3;
				sensordata->hiz_config.reg_setting.size = count_of_hiz_regs;
				sensordata->hiz_config.i2c_addr = hiz_config[0];
				sensordata->hiz_config.reg_setting.addr_type = hiz_config[1];
				sensordata->hiz_config.reg_setting.data_type = hiz_config[2];
				sensordata->hiz_config.reg_setting.delay = hiz_config[3];
				CAM_INFO(CAM_SENSOR, "hiz-config i2c_addr = 0x%02x addr_type = %d data_type = %d delay = %d reg_count = %d", 
					sensordata->hiz_config.i2c_addr,
					sensordata->hiz_config.reg_setting.addr_type,
					sensordata->hiz_config.reg_setting.data_type,
					sensordata->hiz_config.reg_setting.delay,
					count_of_hiz_regs);
				sensordata->hiz_config.reg_setting.reg_setting = (struct cam_sensor_i2c_reg_array *)
					kzalloc(sizeof(struct cam_sensor_i2c_reg_array) * count_of_hiz_regs, GFP_KERNEL);
				if (sensordata->hiz_config.reg_setting.reg_setting != NULL) {
					for (i = 0; i < count_of_hiz_regs; i++) {
							rc = of_property_read_u32_index(of_node, "hiz-regs",
								(3 * i), &temp);
							sensordata->hiz_config.reg_setting.reg_setting[i].reg_addr = temp;
							rc = of_property_read_u32_index(of_node, "hiz-regs",
								(3 * i + 1), &temp);
							sensordata->hiz_config.reg_setting.reg_setting[i].reg_data = temp;
							rc = of_property_read_u32_index(of_node, "hiz-regs",
								(3 * i + 2), &temp);
							sensordata->hiz_config.reg_setting.reg_setting[i].delay = temp;
							CAM_INFO(CAM_SENSOR, "hiz-config reg_addr = 0x%04x reg_data = 0x%04x delay = 0x%2x", 
								sensordata->hiz_config.reg_setting.reg_setting[i].reg_addr,
								sensordata->hiz_config.reg_setting.reg_setting[i].reg_data,
								sensordata->hiz_config.reg_setting.reg_setting[i].delay);
					}

				} else {
					CAM_ERR(CAM_SENSOR, "failed alloc hiz_config.reg_setting");
					sensordata->hiz_config.reg_setting.size = 0;
				}
			} else {
				CAM_ERR(CAM_SENSOR, "data format is not right");
			}
		}
	}
	return rc;
