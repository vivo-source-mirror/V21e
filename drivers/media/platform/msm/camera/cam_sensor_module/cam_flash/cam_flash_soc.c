...
{
	int32_t rc = 0;
	uint32_t count = 0, i = 0, value = 0;
	struct device_node *flash_src_node = NULL;
	struct device_node *torch_src_node = NULL;
	struct device_node *switch_src_node = NULL;

	/*add by vivo luzhi for flash function parse begin*/
	rc = of_property_read_u32(of_node, "rear-flash", &value);
	CAM_ERR(CAM_FLASH, "rear-flash value=%d", value);
	if (value == 1)
		fctrl->flash_function = FUNC_REAR_FLASH;

	value = 0;
	rc = of_property_read_u32(of_node, "front-flash", &value);
	CAM_ERR(CAM_FLASH, "front-flash value=%d", value);
	if (value == 1)
		fctrl->flash_function = FUNC_FRONT_FLASH;

	value = 0;
	rc = of_property_read_u32(of_node, "lcd-flash", &value);
	CAM_ERR(CAM_FLASH, "lcd-flash value=%d", value);
	if (value == 1)
		fctrl->flash_function = FUNC_LCD_FLASH;

	value = 0;
	rc = of_property_read_u32(of_node, "lcd-flash-aux", &value);
	CAM_ERR(CAM_FLASH, "lcd-flash-aux value=%d", value);
	if (value == 1)
		fctrl->flash_function = FUNC_LCD_AUX_FLASH;

	CAM_ERR(CAM_FLASH, "fctrl->flash_function = %d, rc = %d", fctrl->flash_function, rc);
	rc = 0;
	/*add by vivo luzhi for flash function parse end*/
