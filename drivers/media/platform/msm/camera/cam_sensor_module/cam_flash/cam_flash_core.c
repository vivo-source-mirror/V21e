...
int cam_flash_off(struct cam_flash_ctrl *flash_ctrl)
{
	...
	if (flash_ctrl->flash_function != FUNC_LCD_FLASH ||
		flash_ctrl->flash_function == FUNC_LCD_AUX_FLASH) {/*add by vivo wenjun for lcd flash*/
		for (i = 0; i < flash_ctrl->flash_num_sources; i++)
			if (flash_ctrl->flash_trigger[i])
				cam_res_mgr_led_trigger_event(
	...
	struct cam_flash_frame_setting *flash_data)
{
	...
	if (flash_ctrl->flash_function == FUNC_LCD_FLASH ||
		flash_ctrl->flash_function == FUNC_LCD_AUX_FLASH) {
		CAM_INFO(CAM_FLASH, "lcd flash mode >>>   flash low");
		return 0;
	}
	...
	struct cam_flash_frame_setting *flash_data)
{
	...
	if (flash_ctrl->flash_function == FUNC_LCD_FLASH ||
		flash_ctrl->flash_function == FUNC_LCD_AUX_FLASH) {
		CAM_INFO(CAM_FLASH, "lcd flash mode >>>  flash high");
		return 0;
	}
