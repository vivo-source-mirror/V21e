		...
		uint32_t size)
{
	...
	/*luzhi modify memory alloc to avoid tof alloc mem failed of low probability */
	tmp = (struct i2c_settings_list *)
		vzalloc(sizeof(struct i2c_settings_list));
		...
		struct cam_hw_soc_info *soc_info, int gpio_en)
{
	...
	for (i = 0; i < size; i++) {
		CAM_INFO(CAM_SENSOR, "i: %d, gpio %d dir %ld",  i,
	...
	if (gpio_en) {
		for (i = 0; i < size; i++) {
			...
			if (rc) {
				...
				CAM_INFO(CAM_SENSOR, "gpio %d:%s request success",
					gpio_tbl[i].gpio, gpio_tbl[i].label);
			}
		}
	} else {
