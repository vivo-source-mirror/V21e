...
/* luzhi add begin for store cam_thermal ratio information*/
static struct kobject *cam_thermal_kobject;
static struct cam_sensor_ctrl_t *g_sctrl[10];

static ssize_t  cam_thermal0_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
	if (g_sctrl[0]) {
		struct cam_sensor_ctrl_t *sctrl = g_sctrl[0];
		uint32_t temp = (sctrl->sensordata->slave_info.sensor_temperature>>8) & 0x00ff;
		/*environment temp is less than 0, sensor thermal calculate wrong, so do a fool-proof V*/
		if (temp > 80)
			temp = 0;
...
{
	if (g_sctrl[1]) {
		struct cam_sensor_ctrl_t *sctrl = g_sctrl[1];
		uint32_t temp = (sctrl->sensordata->slave_info.sensor_temperature>>8) & 0x00ff;
		/*environment temp is less than 0, sensor thermal calculate wrong, so do a fool-proof */
		if (temp > 80)
			temp = 0;
...
{
	if (g_sctrl[2]) {
		struct cam_sensor_ctrl_t *sctrl = g_sctrl[2];
		uint32_t temp = (sctrl->sensordata->slave_info.sensor_temperature>>8) & 0x00ff;
		/*environment temp is less than 0, sensor thermal calculate wrong, so do a fool-proof V*/
		if (temp > 80)
			temp = 0;
...
{
	if (g_sctrl[3]) {
		struct cam_sensor_ctrl_t *sctrl = g_sctrl[3];
		uint32_t temp = (sctrl->sensordata->slave_info.sensor_temperature>>8) & 0x00ff;
		/*environment temp is less than 0, sensor thermal calculate wrong, so do a fool-proof V*/
		if (temp > 80)
			temp = 0;
...
static void cam_thermal_kobj_init(void)
{
	...
	cam_thermal_kobject = kzalloc(sizeof(*cam_thermal_kobject), GFP_KERNEL);
	if (!cam_thermal_kobject) {
...
/* luzhi add end for store cam_thermal ratio information*/
