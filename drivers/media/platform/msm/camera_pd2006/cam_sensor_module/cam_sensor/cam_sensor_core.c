...
uint32_t totalDpcNum;
uint32_t totalDpcFlag;
...
//for test
	...
	for (j = 0; j < 4; j++) {
	 ...
	 CAM_ERR(CAM_SENSOR, "GC5035_OTP_GC x = %d , y = %d ,type = %d \n",
			((temp_val[1]&0x0f)<<8) + temp_val[0], ((temp_val[2]&0x7f)<<4) + ((temp_val[1]&0xf0)>>4), 
...
static int sensor_gc5035_update_reg(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	for (i = 0; i < 5; i++) {
		if (VALID_FLAG == ((gc5035_chipversion_buffer[CHIPV_OFFSET + 5 * i] >> 3) & 0x01)) {
			...
			CAM_DBG(CAM_SENSOR, "gc5035 otp chipv : 0xfe=0x%x, addr[%d]=0x%x, value[%d]=0x%x", 
...
static void cam_sensor_read_thermal(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (read_count > 30) {
		read_count = 0;
		if (s_ctrl->sensordata->slave_info.thermal_addr != 0x0000) {
			camera_io_dev_read(&(s_ctrl->io_master_info),
				s_ctrl->sensordata->slave_info.thermal_addr, &read_data,
				s_ctrl->sensor_probe_addr_type, s_ctrl->sensor_probe_data_type);
			s_ctrl->sensordata->slave_info.sensor_temperature = read_data;
		} else {
			s_ctrl->sensordata->slave_info.sensor_temperature = 0;
		}
		CAM_INFO(CAM_SENSOR, "sensor_thermal_addr=0x%4x sensor_temperature=0x%4x", s_ctrl->sensordata->slave_info.thermal_addr,
			s_ctrl->sensordata->slave_info.sensor_temperature);
	}
}
	...
	struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (probe_flag > 0) {
		...
		s_ctrl->sensordata->slave_info.thermal_addr = probe_info->thermal_addr;
	}
...
/* gaoshang add for make wide hi846 soft-standby begin*/
struct cam_sensor_i2c_reg_array reg_standby[] = {
	{0x0900, 0x0301, 0x00},	 //mipi Hi-Z
};

static int cam_sensor_enter_hi846_standby(struct cam_sensor_ctrl_t *s_ctrl)
{
	int rc = 0;
	struct cam_sensor_i2c_reg_setting i2c_settings;
	uint16_t sid_temp;

	i2c_settings.reg_setting = reg_standby;
	i2c_settings.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
	i2c_settings.data_type = CAMERA_SENSOR_I2C_TYPE_WORD;
	i2c_settings.size = ARRAY_SIZE(reg_standby);
	i2c_settings.delay = 0;

	sid_temp = s_ctrl->io_master_info.cci_client->sid;
	s_ctrl->io_master_info.cci_client->sid = 0x42>>1;
	rc = camera_io_dev_write(&(s_ctrl->io_master_info), &i2c_settings);
	s_ctrl->io_master_info.cci_client->sid = sid_temp;

	CAM_INFO(CAM_SENSOR, "let hi846 enter standby mode rc=%d", rc);

	return rc;
}
/* gaoshang add for make wide hi846 soft-standby end */
	...
	void *arg)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
		...
		for (i = 0; i < 3; i++) { //vivo dongzhixiang add to reprobe if for cci error 2019/4/11
			...
			if (rc < 0) {
				cam_sensor_power_down(s_ctrl);
				msleep(20);
				CAM_ERR(CAM_SENSOR, "reprobe i = %d", i);
				if (i == 2 || rc != -EINVAL)
					goto free_power_settings;
			} else {
				break;
	...
	case CAM_START_DEV: {
		...
		/*gaoshang add begin for two camera share the same mipi phy and another one should enter standby to let mipi pin go into high-ohm status*/
		if (s_ctrl->should_standby == 1) {
			cam_sensor_enter_hi846_standby(s_ctrl);
		}
		/*gaoshang add end for two camera share the same mipi phy and another one should enter standby to let mipi pin go into high-ohm status*/
	...
	case CAM_CONFIG_DEV: {
		...
		if (s_ctrl->i2c_data.init_settings.request_id != 0 &&
			s_ctrl->sensordata->slave_info.thermal_addr > 0)
...
int cam_sensor_power_up(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->cam_vdd_regulator != NULL) {
		regulator_set_voltage(s_ctrl->cam_vdd_regulator,
			s_ctrl->cam_vdd_target_voltage, s_ctrl->cam_vdd_target_voltage);
	}
