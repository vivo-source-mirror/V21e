...
static int32_t cam_sensor_driver_get_dt_data(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	//vivo gaoshang add to support sharded-mipi 2020/3/30
	if (of_property_read_u32(of_node, "another-sensor-should-standby",
		&s_ctrl->should_standby) < 0) {
		CAM_INFO(CAM_SENSOR, "index=%d should_standby=%d", s_ctrl->soc_info.index, s_ctrl->should_standby);
	}
	//vivo gaoshang add to support sharded-mipi 2020/3/30
	...
	if (src_node != NULL) {
		...
		&s_ctrl->cam_vdd_max_voltage) >= 0) {
			...
			if (of_property_read_u32(of_node, "cam_vdd-target-voltage",
				&s_ctrl->cam_vdd_target_voltage) < 0) {
				s_ctrl->cam_vdd_target_voltage = s_ctrl->cam_vdd_max_voltage;
				}
