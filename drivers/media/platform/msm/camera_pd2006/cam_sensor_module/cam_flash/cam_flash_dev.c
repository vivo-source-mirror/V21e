...
static struct cam_flash_ctrl *g_fctrl[MAX_FLASHS];  /*add by vivo wenjun for ext led_flash node 2017.02.27*/
static struct kobject *ext_flash_led_kobject;



/*************************************************************
*   add by vivo wenjun for ext led_flash node start
*
*   version 1.1 :front flash and rear flash have independent switch nodes
*
*   2017.02.27
**************************************************************/
						...
						struct kobj_attribute *attr, char *buf)
{
   ...
   if (0 == strcmp(attr->attr.name, "EXT_FLASH_LED"))
		flash_ctrl = g_fctrl[FLASH_REAR];
	else if (0 == strcmp(attr->attr.name, "EXT_FRONT_FLASH_LED"))
		flash_ctrl = g_fctrl[FLASH_FRONT];

	if (!flash_ctrl) {
		CAM_ERR(CAM_FLASH, "%s:%d failed flash_ctrl is NULL", __func__, __LINE__);
		return -EINVAL;
	}
...
static ssize_t  ext_rear_flash_dual_led_store(struct kobject *kobj,
						struct kobj_attribute *attr, const char *buf, size_t size)
{
	uint32_t rc =  0, i;
	uint32_t flash_current = 0;
	char str_flash_current[10];
	bool flash_mode;    /* flag for flash on or off*/
	struct cam_flash_ctrl *flash_ctrl = g_fctrl[FLASH_REAR];
	...
	snprintf(str_flash_current, sizeof(str_flash_current), "%s", buf);
	rc = kstrtou32(str_flash_current, 0, &flash_current);
	flash_mode = (flash_current > 0) ? 1:0;

	CAM_ERR(CAM_FLASH, "set flash_current = (%d) ; name %s ; flash_mode = %d",
		flash_current, attr->attr.name, flash_mode);

	for (i = 0; i < flash_ctrl->torch_num_sources; i++) {
		if (flash_ctrl->torch_trigger[i])
			cam_res_mgr_led_trigger_event(flash_ctrl->torch_trigger[i], flash_current);
		CAM_DBG(CAM_FLASH, "led_trigger_event: torch_trigger[%d] = %d ", i, flash_current);
		usleep_range(1000, 1050);
	}

	if (flash_ctrl->switch_trigger)
		cam_res_mgr_led_trigger_event(flash_ctrl->switch_trigger, flash_mode);

	return size;
}

static ssize_t  ext_rear_flash_single_led_store(struct kobject *kobj,
						struct kobj_attribute *attr, const char *buf, size_t size)
{
	uint32_t rc =  0;
	uint32_t flash_current = 0;
	...
	if (0 == strcmp(attr->attr.name, "EXT_REAR_FLASH_LED1")) {
		current_torch = 0;
	} else if (0 == strcmp(attr->attr.name, "EXT_REAR_FLASH_LED2")) {
		if (flash_ctrl->torch_num_sources > 1)
			current_torch = 1;
		else
			current_torch = 0;
	}
	...
	if (flash_ctrl->torch_trigger[current_torch]) {
		cam_res_mgr_led_trigger_event(flash_ctrl->torch_trigger[current_torch], flash_current);
		CAM_ERR(CAM_FLASH, "led_trigger_event: torch_trigger[%d] = %d ", current_torch, flash_current);
		usleep_range(1000, 1050);
	}
...
static ssize_t  ext_front_flash_led_store(struct kobject *kobj,
						struct kobj_attribute *attr, const char *buf, size_t size)
{
	uint32_t i;
	uint32_t flash_current = 0;
	...
	struct cam_flash_ctrl *flash_ctrl = g_fctrl[FLASH_FRONT];
	if (!flash_ctrl) {
		CAM_ERR(CAM_FLASH, "%s:%d failed flash_ctrl is NULL", __func__, __LINE__);
		return -EINVAL;
	}
	...
	snprintf(str_flash_current, sizeof(str_flash_current), "%s", buf);
	i = kstrtou32(str_flash_current, 0, &flash_current);
	flash_mode = (flash_current > 0) ? 1:0;

	CAM_ERR(CAM_FLASH, "set flash_current = (%d) ; name %s ; flash_mode = %d torch_num_sources=%d",
		flash_current, attr->attr.name, flash_mode, flash_ctrl->torch_num_sources);

	if (flash_current > 175) {
			for (i = 0; i < flash_ctrl->flash_num_sources; i++) {
				if (flash_ctrl->flash_trigger[i]) {
					cam_res_mgr_led_trigger_event(flash_ctrl->flash_trigger[i], flash_current);
					CAM_ERR(CAM_FLASH, "led_trigger_event: flash_trigger[0] = %d ", flash_current);
					usleep_range(1000, 1050);
				}
			}
			if (flash_ctrl->switch_trigger)
	...
	CAM_ERR(CAM_FLASH, "set flash_current end");

	return size;
}

static struct kobj_attribute ext_flash_led1_attribute =
__ATTR(EXT_REAR_FLASH_LED1, 0644, ext_flash_led_show, ext_rear_flash_single_led_store);

static struct kobj_attribute ext_flash_led2_attribute =
__ATTR(EXT_REAR_FLASH_LED2, 0644, ext_flash_led_show, ext_rear_flash_single_led_store);

static struct kobj_attribute ext_flash_led_attribute =
__ATTR(EXT_REAR_FLASH_LED, 0644, ext_flash_led_show, ext_rear_flash_dual_led_store);

static struct kobj_attribute ext_front_flash_led_attribute =
__ATTR(EXT_FRONT_FLASH_LED, 0644, ext_flash_led_show, ext_front_flash_led_store);
...
static int ext_flash_led_init(void)
{
	CAM_DBG(CAM_FLASH, "Enter\n");
	ext_flash_led_kobject = kzalloc(sizeof(*ext_flash_led_kobject), GFP_KERNEL);
	if (!ext_flash_led_kobject) {
...
/**********************************************************
*   add by vivo wenjun for led_flash end
*
*		2017.02.27
***********************************************************/
...
static int create_flash_led_attribute(struct cam_flash_ctrl *flash_ctrl) 
{
	int32_t rc = 0;
	CAM_ERR(CAM_FLASH, "flash_ctrl->flash_function=%d", flash_ctrl->flash_function);
	switch (flash_ctrl->flash_function) {
	case FUNC_REAR_FLASH:
		if (g_fctrl[FLASH_REAR] == NULL) {
			g_fctrl[FLASH_REAR] = flash_ctrl;
			rc = sysfs_create_file(ext_flash_led_kobject, &ext_flash_led_attribute.attr);
			if (flash_ctrl->flash_num_sources == 2) {
				rc = sysfs_create_file(ext_flash_led_kobject, &ext_flash_led1_attribute.attr);
				rc = sysfs_create_file(ext_flash_led_kobject, &ext_flash_led2_attribute.attr);
			}
		}
		break;
	case FUNC_FRONT_FLASH:
		if (g_fctrl[FLASH_FRONT] == NULL) {
			g_fctrl[FLASH_FRONT] = flash_ctrl;
			rc = sysfs_create_file(ext_flash_led_kobject, &ext_front_flash_led_attribute.attr);
		}
		break;
	case FUNC_LCD_FLASH:
		if (g_fctrl[FLASH_LCD] == NULL) {
		g_fctrl[FLASH_LCD] = flash_ctrl;
		}
		break;
	default:
		pr_err("The flash cell-index is invalid , out of MAX_FLASHS %d\n", MAX_FLASHS);
	}
	return rc;
}
...
static int32_t cam_flash_platform_probe(struct platform_device *pdev)
{
	...
	create_flash_led_attribute(fctrl);
