...
void cam_csiphy_shutdown(struct csiphy_device *csiphy_dev)
{
	...
	/* reset csiphy info */
	CAM_INFO(CAM_CSIPHY, "reset the lane_mask, lane_ct combo_mode to 0");
			...
			void *arg)
{
	...
	case CAM_RELEASE_DEV: {
		...
		if (csiphy_dev->config_count == 0) {
			...
			CAM_INFO(CAM_CSIPHY, "config_count : %d lane_mask : %x lane_cnt : %d , combo_mode: %d",
					csiphy_dev->config_count, csiphy_dev->csiphy_info.lane_cnt,
					csiphy_dev->csiphy_info.lane_cnt, csiphy_dev->csiphy_info.combo_mode);
		}
	}
		break;
	case CAM_CONFIG_DEV: {
