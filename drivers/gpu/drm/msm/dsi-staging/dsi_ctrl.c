...
static irqreturn_t dsi_ctrl_isr(int irq, void *ptr)
{
	...
	if (status & DSI_CMD_MODE_DMA_DONE) {
		...
		if (atomic_read(&vdisp->hbm_wait_te_status) == DSI_PANEL_WAIT_HBM_CMD_DONE) {
			atomic_set(&vdisp->hbm_wait_te_status, DSI_PANEL_WAIT_ONE_TE);
			pr_err("%s:set hbm_wait_te_status = %d", __func__, atomic_read(&vdisp->hbm_wait_te_status));
		}
		dsi_ctrl_disable_status_interrupt(dsi_ctrl,
					DSI_SINT_CMD_MODE_DMA_DONE);
		complete_all(&dsi_ctrl->irq_info.cmd_dma_done);
