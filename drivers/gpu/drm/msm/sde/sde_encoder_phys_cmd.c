...
extern struct panel_ud_hbm_sync *get_ud_hbm_sync(void);
static void sde_encoder_phys_cmd_pp_tx_done_irq(void *arg, int irq_idx)
{
	...
	struct panel_ud_hbm_sync *ud_hbm_sync = get_ud_hbm_sync();
	struct vivo_display *vdisp = get_vivo_vdisp();

	if (vdisp && ud_hbm_sync && vdisp->is_ud_fingerprint && ud_hbm_sync->wait_pp_done) {
			ud_hbm_sync->wait_pp_done = 0;
			pr_info("%s:get pp done\n", __func__);
			queue_work(ud_hbm_sync->hbm_workq, &ud_hbm_sync->aod_on_work);
	}
