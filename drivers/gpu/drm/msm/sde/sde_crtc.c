...
static void sde_crtc_vblank_cb(void *data)
{
	...
	SDE_ATRACE_BEGIN("Notify vblank");
	sysfs_notify_dirent(sde_crtc->vsync_event_sf);
	SDE_ATRACE_END("Notify vblank");
...
void sde_fingerprint_notify_work(struct work_struct *work)
{
	...
	pr_info("start notify fingershooter state: 0x%x\n", sde_crtc->fingershooter_status);
...
int sde_notify_fingerprint_status(struct drm_crtc *crtc)
{
	...
	if (need_notify) {
		...
		pr_info("prepare notify fingershooter state: 0x%x\n", sde_crtc->fingershooter_status);
