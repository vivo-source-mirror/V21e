...
static void _bimc_clk_prepare_enable(struct kgsl_device *device,
				struct clk *clk, const char *name);
/*port by dingbo for vivo_pem begin*/
#ifdef CONFIG_VIVO_STATISTICS_BUSY_TIME
struct mt_gpu_power_stat{
	spinlock_t lock; /*lock for update this struct data*/
	bool active; /*is gpu active*/
	int lastFreq; /*last gpu freq*/
	ktime_t time_period_start;/*last update time*/
	long long int *activeTime;/*gpu active time on every freq*/
	int num_pwrlevel;
};
static struct mt_gpu_power_stat g_gpu_power_stat;

void mt_gpufreq_power_stat_init(int num_pwrlevel, int active_pwrlevel)
{
	int i = 0;
	spin_lock_init(&g_gpu_power_stat.lock);
	g_gpu_power_stat.active = true;
	g_gpu_power_stat.num_pwrlevel = num_pwrlevel;
	g_gpu_power_stat.lastFreq = active_pwrlevel;
	g_gpu_power_stat.time_period_start = ktime_get();
	if (g_gpu_power_stat.activeTime != NULL)
		kfree(g_gpu_power_stat.activeTime);
	g_gpu_power_stat.activeTime = kzalloc((num_pwrlevel) * sizeof(long long int), GFP_KERNEL);
	for (i = 0; i < num_pwrlevel; i++)
		g_gpu_power_stat.activeTime[i] = 0;

}


/*call when gpu freq change, stat the active time on all freq*/
static void note_switch_gpu_freq(int active_pwrlevel, u64 active_time)
{
	unsigned long flags;
	spin_lock_irqsave(&g_gpu_power_stat.lock, flags);
	g_gpu_power_stat.activeTime[active_pwrlevel] += active_time*1000;
	spin_unlock_irqrestore(&g_gpu_power_stat.lock, flags);
}


/*show freq info on /sys/class/kgsl/kgsl-3d0/gpu_active_time*/
static ssize_t kgsl_pwrctrl_gpu_active_time_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int i = 0;
	int loffset = 0;
	...
	for (i = 0; i < g_gpu_power_stat.num_pwrlevel; i++)
		loffset += snprintf(buf + loffset, PAGE_SIZE, "%llu ", (long long unsigned)(g_gpu_power_stat.activeTime[i]));
	loffset += snprintf(buf + loffset, PAGE_SIZE, "%s ", "\n");
	return loffset;
}
#endif
/*port by dingbo for vivo_pem end*/

/*port by jingruiguang for vivo_pem GPU power optimize begin*/
#ifdef CONFIG_VIVO_OPTIMIZE_GPU_POWER
static ssize_t kgsl_pwrctrl_optimize_level_show(struct device *dev,
...
static ssize_t kgsl_pwrctrl_optimize_level_store(struct device *dev,
					 struct device_attribute *attr,
...
#endif
/*port by jingruiguang for vivo_pem GPU power optimize end*/
...
/*port by dingbo for vivo_pem begin*/
#ifdef CONFIG_VIVO_STATISTICS_BUSY_TIME
static DEVICE_ATTR(gpu_active_time, 0444,
	kgsl_pwrctrl_gpu_active_time_show,
	NULL);
#endif
/*port by dingbo for vivo_pem end*/
/*port by jingruiguang for vivo_pem begin*/
#ifdef CONFIG_VIVO_OPTIMIZE_GPU_POWER
static DEVICE_ATTR(optimize_level, 0644,
	kgsl_pwrctrl_optimize_level_show,
	kgsl_pwrctrl_optimize_level_store);
#endif
/*port by jingruiguang for vivo_pem end	*/
...
/*port by dingbo for vivo_pem begin*/
#ifdef CONFIG_VIVO_STATISTICS_BUSY_TIME
	&dev_attr_gpu_active_time,
#endif
/*port by dingbo for vivo_pem end*/
/*port by jingruiguang for vivo_pem begin*/
#ifdef CONFIG_VIVO_OPTIMIZE_GPU_POWER
	&dev_attr_optimize_level,
#endif
/*port by jingruiguang for vivo_pem end*/
...
void kgsl_pwrctrl_busy_time(struct kgsl_device *device, u64 time, u64 busy)
{
	...
	stats->total = 0;
	stats->busy = 0;
/*port by dingbo for vivo_pem beign*/
#ifdef CONFIG_VIVO_STATISTICS_BUSY_TIME
	note_switch_gpu_freq(device->pwrctrl.active_pwrlevel, stats->busy_old);
#endif
/*port by dingbo for vivo_pem end*/
...
/*port by dingbo for vivo_pem begin*/
#ifdef CONFIG_VIVO_STATISTICS_BUSY_TIME
	mt_gpufreq_power_stat_init(pwr->num_pwrlevels, pwr->active_pwrlevel);
#endif
/*port by dingbo for vivo_pem end*/
...
/*port by jingruiguang for vivo_pem GPU power optimize begin*/
#ifdef CONFIG_VIVO_OPTIMIZE_GPU_POWER
void kgsl_set_min_freq(struct kgsl_device *device)
{
	struct kgsl_pwrctrl *pwr;
	if (device == NULL)
		return ;
	pwr = &device->pwrctrl;
	if (!pwr->optimize_flag) {
		pwr->active_max_pwrlevel = pwr->max_pwrlevel;
		pwr->max_pwrlevel = pwr->min_pwrlevel;
		kgsl_pwrctrl_pwrlevel_change(device, pwr->active_pwrlevel);
		pwr->optimize_flag = true;
	}
}

void kgsl_reset_org_freq(struct kgsl_device *device)
{
	struct kgsl_pwrctrl *pwr;
	if (device == NULL)
		return ;
	...
	if (pwr->optimize_flag) {
		pwr->max_pwrlevel = pwr->active_max_pwrlevel;
		kgsl_pwrctrl_pwrlevel_change(device, pwr->active_pwrlevel);
		pwr->optimize_flag = false;
	}
}
#endif
/*port by jingruiguang for vivo_pem GPU power optimize end*/
...
/*port by jingruiguang for vivo_pem GPU power optimize begin*/
#ifdef CONFIG_VIVO_OPTIMIZE_GPU_POWER
	if (device->state == KGSL_STATE_ACTIVE && device->pwrctrl.active_pwrlevel < device->pwrctrl.optimize_level) {
		kgsl_set_min_freq(device);
	} else if (state == KGSL_STATE_ACTIVE && device->pwrctrl.optimize_flag) {
		kgsl_reset_org_freq(device);
	}
#endif
/*port by jingruiguang for vivo_pem GPU power optimize end*/
