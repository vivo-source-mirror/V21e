	...
	if (!dynamic || (smmu->options & ARM_SMMU_OPT_NO_DYNAMIC_ASID)) {
		...
		if (ret < 0) {
			...
			if (strstr(dev_name(smmu->dev), "smmu-kgsl") != NULL) {
				dev_err(smmu->dev, "dynamic ASID allocation failed: kgsl_smmu has been alloced %d times, smmu->num_context_banks = %d, MAX_ASID =%d pid: %d,comm :%s \n", kgsl_smmu_num, smmu->num_context_banks, MAX_ASID, current->pid,current->comm);
			}
#endif
			return ret;
		}
		...
		if (strstr(dev_name(smmu->dev), "smmu-kgsl") != NULL) {
			kgsl_smmu_num++;
			pr_err("arm_smmu_init_asid, after kgsl_smmu_num++, kgsl_smmu_num: %d,pid: %d,comm :%s \n", kgsl_smmu_num,current->pid,current->comm);
	    }
#endif
	}
	return 0;
