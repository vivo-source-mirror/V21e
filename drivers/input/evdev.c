 ...
 */
static void evdev_events_timestamp(struct input_handle *handle,
			 const struct input_value *vals, unsigned int count, ktime_t kt)
{
	struct evdev *evdev = handle->private;
	struct evdev_client *client;
	ktime_t ev_time[EV_CLK_MAX];

	ev_time[EV_CLK_MONO] = kt;
...
static void evdev_event_timestamp(struct input_handle *handle,
			unsigned int type, unsigned int code, int value, ktime_t timestamp)
{
	struct input_value vals[] = { { type, code, value } };

	evdev_events_timestamp(handle, vals, 1, timestamp);
}

static void evdev_events(struct input_handle *handle,
			 const struct input_value *vals, unsigned int count)
{
	evdev_events_timestamp(handle, vals, count, ktime_get());
}
/*
...
static struct input_handler evdev_handler = {
	.events_timestamp = evdev_events_timestamp,
	.event_timestamp = evdev_event_timestamp,
	.event		= evdev_event,
	.events		= evdev_events,
