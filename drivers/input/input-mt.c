...
void input_mt_report_slot_state_timestamp(struct input_dev *dev,
				unsigned int tool_type, bool active, ktime_t timestamp)
{
	...
	if (!active) {
		input_event_timestamp(dev, EV_ABS, ABS_MT_TRACKING_ID, -1, timestamp);
		return;
	}
	...
	input_event_timestamp(dev, EV_ABS, ABS_MT_TRACKING_ID, id, timestamp);
	input_event_timestamp(dev, EV_ABS, ABS_MT_TOOL_TYPE, tool_type, timestamp);
}
EXPORT_SYMBOL(input_mt_report_slot_state_timestamp);
