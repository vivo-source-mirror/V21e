...
static void qti_haptics_enable(struct timed_output_dev *sdev, int timeout)
{
	...
	int rc = 0;
	HAPTIC_LOG("enter, timeout is %d\n", timeout);
	if (timeout <= 0) {
		pr_err("constant play time must beyond 0\n");
		return;
	}
	mutex_lock(&chip->enable_lock);
	if (test->factory_test_en == false) {
		pr_info("the factory test is begin\n");
		...
		rc = qti_haptics_load_constant_waveform(chip);
		if (rc < 0) {
			pr_err("Play constant waveform failed, rc=%d\n", rc);
			mutex_unlock(&chip->enable_lock);
		...
		pr_err("the factory test is running\n");
...
static int qti_haptics_get_time(struct timed_output_dev *sdev)
{
	...
	HAPTIC_LOG("enter, test play times = %u\n", chip->test.have_test_times);
...
static const struct of_device_id haptics_match_table[] = {
	{ .compatible = "qcom,haptics-erm" },
	{ .compatible = "qcom,pm660-haptics-erm" },
	{ .compatible = "qcom,pm8150b-haptics-erm" },
	{},
};

static struct platform_driver qti_haptics_driver = {
	.driver		= {
		.name = "qcom,haptics-erm",
		.owner = THIS_MODULE,
