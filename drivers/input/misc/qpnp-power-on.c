...
struct qpnp_pon {
	...
	/* Begin add kpdpwr monitor leiweiqiang 2017-08-28*/
	struct delayed_work kpdpwr_work;
	/* End add kpdpwr monitor leiweiqiang 2017-08-28*/
	u16			base;
	u8			subtype;
	u8			pon_ver;
...
/* Begin leiweiqiang modify Volume down key dependent on bsp_test_mode 2016-10-12*/
...
/* End leiweiqiang modify Volume down key dependent on bsp_test_mode 2016-10-12*/
...
/* Begin leiweiqiang add the kpdpwr_resin set dloadmode to debug 2016-01-16*/
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	qpnp_pon_kpdpwr_resin_cfg(pon, cfg, !!value);
...
/* End leiweiqiang add the kpdpwr_resin set dloadmode to debug 2016-01-16*/
...
void vivo_wdt_monitor_prepare_and_start(void)
{
	...
	/*???
	 *g_force_offline_init???????wdt_monitor??完?????????:
	 *1??????vivolog??wdt????完
	 *2????????(eng??bsptmode)??wdt????完
	 *3?????force_offline???????????wdt????完
	 *4????force_offline??忌??????????㏒??????wdt_test????????完
	 */
...
/*Begin leiweiqiang add Monitor kpdpwr state 2017-08-08*/
...
/*End leiweiqiang add Monitor kpdpwr state 2017-08-08*/
					    ...
					    struct device_node *node)
{
	...
	/* Begin leiweiqiang modify KPDPWR_RESIN dependent on bsp_test_mode 2016-11-22*/
	if (bsp_test_mode &&
			(PON_KPDPWR_RESIN == cfg->pon_type)) {
		/* Support KPDPWR_RESIN comeinto dump*/
		cfg->s2_type = 1;
		pr_err("s2_type:%d bsp_test_mode:%d\n",
				cfg->s2_type, bsp_test_mode);
	}
	/* End leiweiqiang modify KPDPWR_RESIN dependent on bsp_test_mode 2016-11-22*/
...
static int qpnp_pon_probe(struct platform_device *pdev)
{
	...
	/* Begin leiweiqiang Monitor kpdpwr state*/
	INIT_DELAYED_WORK(&pon->kpdpwr_work, kpdpwr_monitor_work_func);
	/* End leiweiqiang Monitor kpdpwr state*/
	...
	/*Begin leiweiqiang add set dloadmode to debug*/
	rc = device_create_file(dev, &dev_attr_kpdpwr_resin_s2_type);
	if (rc) {
		dev_err(dev, "sys file kpdpwr_resin_s2_type creation failed\n");
		return rc;
	}

	rc = device_create_file(dev, &dev_attr_kpdpwr_monitor_state);
	if (rc) {
		dev_err(dev, "sys file kpdpwr_monitor_state creation failed\n");
		return rc;
	}

	rc = device_create_file(dev, &dev_attr_kpdpwr_monitor_timeout);
	if (rc) {
		dev_err(dev, "sys file monitor_timeout_secs creation failed\n");
		return rc;
	}
	/*End leiweiqiang add set dloadmode to debug*/
