...
static unsigned long adsp_restart_timeout;
...
hw_init_retry:
	...
	if (!mdm_restart &&
		...
		if (ret) {
			...
			if ((retries == INIT_MX_RETRIES) &&
				!atomic_read(&dev->ssr_in_progress) &&
				time_after(jiffies, adsp_restart_timeout)) {
				pr_err("%s: SLIM power req failed, restart adsp\n", __func__);
				subsystem_restart("adsp");
				adsp_restart_timeout = jiffies + HZ * 30;
			}
			return ret;
		}
	}
...
static void ngd_dom_up(struct work_struct *work)
{
	...
	do {
		...
		retry--;
		SLIM_WARN(dev, "ngd_dom_up: slim ret %d retry %d\n", ret, retry);
	} while (ret && (retry > 0));
	mutex_unlock(&dev->ssr_lock);
}
