...
static unsigned int bias_ctrl2;
module_param(bias_ctrl2, uint, 0644);
MODULE_PARM_DESC(bias_ctrl2, "QUSB PHY BIAS_CTRL2");
...
struct qusb_phy {
	...
	int			efuse_correction;
	int			efuse_host_correction;
...
static void qusb_phy_get_tune1_param(struct qusb_phy *qphy)
{
	...
	tune_val_correction = qphy->tune_val;
	if (qphy->phy.flags & PHY_HOST_MODE)
		tune_val_correction += qphy->efuse_host_correction;
	else
		tune_val_correction += qphy->efuse_correction;

	if (tune_val_correction < 0)
		tune_val_correction = 0;
	if (tune_val_correction > bit_mask)
		tune_val_correction = bit_mask;

	pr_debug("(mode: %d)efuse=%d, correction=%d, value=%d\n", !!(qphy->phy.flags & PHY_HOST_MODE), qphy->tune_val,
		!!(qphy->phy.flags & PHY_HOST_MODE) ? qphy->efuse_host_correction : qphy->efuse_correction, tune_val_correction);

	reg = readb_relaxed(qphy->base + qphy->phy_reg[PORT_TUNE1]);
	if (qphy->tune_val) {
		reg = reg & 0x0f;
		reg |= (tune_val_correction << 4);
	}
...
static void qusb_phy_host_init(struct usb_phy *phy)
{
	...
	if (tune4)
		writel_relaxed(tune4,
			qphy->base + qphy->phy_reg[PORT_TUNE1] + 12);
	if (tune5)
		writel_relaxed(tune5,
			qphy->base + qphy->phy_reg[PORT_TUNE1] + 16);
	...
	if (qphy->refgen_north_bg_reg && qphy->override_bias_ctrl2) {
		if (readl_relaxed(qphy->refgen_north_bg_reg) & BANDGAP_BYPASS) {
			...
			pr_debug("%s(): Programming BIAS_CTRL_2 parameter as:%x\n",
				 __func__, BIAS_CTRL_2_OVERRIDE_VAL);
		}
	}
	...
	if (bias_ctrl2)
		writel_relaxed(bias_ctrl2,
				qphy->base + qphy->phy_reg[BIAS_CTRL_2]);

	dev_dbg(phy->dev, "(mode: %d)tune1:%08x tune2:%08x tune3:%08x tune4:%08x tune5:%08x bias1:%08x bias2:%08x\n", !!(qphy->phy.flags & PHY_HOST_MODE),
		...
		readl_relaxed(qphy->base + qphy->phy_reg[BIAS_CTRL_2] - 4),
	...
	if (res) {
		...
		if (!IS_ERR_OR_NULL(qphy->efuse_reg)) {
			...
			if (of_property_read_u32(dev->of_node,
						 "qcom,efuse-correction",
						 &qphy->efuse_correction))
				qphy->efuse_correction = 0;
			if (of_property_read_u32(dev->of_node,
						 "qcom,efuse-host-correction",
						 &qphy->efuse_host_correction))
				qphy->efuse_host_correction = 0;
		}
	}
