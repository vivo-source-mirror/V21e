...
static ssize_t nfc_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	else if (nfc_result == 2)
		return snprintf(buf, 2, "%d", 2);
	else
...
static int nfc_parse_dt(struct device *dev, struct nqx_platform_data *pdata)
{
	...
	for (i = 0; i < boardversion_num; i++)
		printk("nfc :%s\n", boardversions[i]);
	ret = of_property_read_u32(np, "vivo,nfc_chiptype", &nfc_chiptype);
	if (ret < 0) {
		dev_err(dev, "Failure reading vivo,nfc_chiptype, ret = %d\n", ret);
		nfc_chiptype = 0;
	}
	printk("nfc nfc_chiptype:%d\n", nfc_chiptype);
...
static void nfc_chip_type(struct device *dev)
{
	if (1 == nfc_result) {
		switch (nfc_chiptype) {
		case 0:
			dev_err(dev, "unkownchip or parse failed!\n");
			break;
		case 1:
			dev_err(dev, "chip with ese\n");
			break;
		case 2:
			dev_err(dev, "chip without ese\n");
			nfc_result = nfc_chiptype;
			break;
		default:
			dev_err(dev, "no chip\n");
		}
