...
static int proc_ion_mm_heap_open(struct inode *inode, struct file *file)
{
	return single_open(file, dma_buf_debug_show, inode->i_private);
}

static const struct file_operations proc_ion_mm_heap_fops = {
	.open = proc_ion_mm_heap_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static int ion_mm_heap_init_procfs(void)
{
	struct proc_dir_entry *proc_file;
	proc_root = proc_mkdir("ion", NULL);
	if (proc_root) {
		pr_err("ion: failed to create ion procfs root directory.\n");
	}

	/*
	 * This node is for userspace read.
	 * In order to align with mtk, witch name is changed to ion_mm_heap
	 */
	proc_file = proc_create("ion_mm_heap", S_IFREG | 0644,
				     proc_root, &proc_ion_mm_heap_fops);
	if (!proc_file)
		pr_err("Failed to create procfs at /proc/ion/ion_mm_heap\n");

	return 0;
}

static int ion_mm_heap_uninit_procfs(void)
{
