...
#if defined(CONFIG_SERIAL_CORE_CONSOLE) || defined(CONFIG_CONSOLE_POLL)
static  msm_geni_console_setup(struct console *co, char *options)
{
	struct uart_port *uport;
...
int msm_geni_iacore_uart_pinctrl_enable(int enable)
{
	...
	if (IS_ERR_OR_NULL(rsc->geni_pinctrl) ||
		IS_ERR_OR_NULL(rsc->geni_gpio_active)) {
	...
	pr_info("%s: iacore uart: %s, wakeup_irq: %d, uport_irq: %d.\n",
				__func__, port->name, port->wakeup_irq, port->uport.irq);
	return ret;
...
static int msm_geni_serial_runtime_suspend(struct device *dev)
{
	...
	dev_info(dev, "%s: iacore uart: %s, wakeup_irq: %d, uport_irq: %d.\n",
			__func__, port->name, port->wakeup_irq, port->uport.irq);
